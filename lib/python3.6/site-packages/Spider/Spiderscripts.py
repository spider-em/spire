...
re_section = re.compile(r'# *--+ *[ \w]+ *--+')   # patterns: "# --- word ---"
...
def putExtension(ext, force=0):
    ...
    for i in range(nlines):
        hdr.append(linecache.getline(filename, i))
    vkeys = list(vardict.keys())
...
def useFilenums(filenums, template):
    ...
    if type(filenums) == type("string"):
        if os.path.exists(filenums):
            filenums = Spiderutils.readdoc(filenums)
    for i in filenums:
        files.append(Spiderutils.makeSpiderFilename(template, i))
    return files
...
def isParmDataLine(line):
    ...
    if len(d) < 3:
        return 0
    try:
        int(float(d[0]))  #### previously assigned to 'key'
        num = int(d[1])
        if num != 1:
            return 0
        float(d[2])  #### previously assigned to 'val'
        return 1
    except:
        return 0
