    ...
    # if keys != 0, return a dictionary
        ...
        if keys == 'all':
            ...
            for line in B:
                ...
                key = int(d[0])
                ncols = int(d[1]) + 1
                data = list(map(float,d[2:ncols+1]))
                D[key] = data
        else:
...
def nowisthetime():
    ...
    d = list(map(str,tt))   # stringify all numbers in the tuple
    mon = d[1]
    ...
    if columns != None:
        ...
        if keys == None:
            if lastkey == None:
                ...
                keys = list(range(lastkey+1, lastkey+n+1))
...
def list2int(a):
    ...
    return list(map(int, list(map(float,a))))
...
re_nam = re.compile(r'\[[ a-zA-Z0-9_-]+\] *=') # "[symbol] =" patterns
re_sym = re.compile(r'\?[ \w]+\?')   # ?text? patterns

# top line of procedures
re_reglist = re.compile('([xX][0-9][0-9])(, *[xX][0-9][0-9])*') # x11,x12,x13
re_namlist = re.compile(r'(\[[ a-zA-Z0-9_-]+\])(, *\[[ a-zA-Z0-9_-]+\])*')
re_nam1 = re.compile(r'(\[[ a-zA-Z0-9_-]+\])')  # named reg in proc hdr
...
def testSpider(spider):
    ...
    success = 0
    output = subprocess.getoutput(spicmd)
...
def printvars(variables, quitTF=False, typeTF=False):
    ...
    if type(variables) is list:
        ...
        variable_list= [strings]
    elif isinstance(variables, six.string_types):  # six works with both Pythons 2 & 3
        variable_list= [variables]
    else:
        print("WARNING! printvars:1420: Don't know how to deal with type %s" % type(variables) )
        return
...
def whocalledme(num2print, strings=None, values=None):
    """
    Purpose:
      Shows what function called what other function, or
        shows one or more variables along with what program+function produced them.

      The number to print is arbitrary, but I typically use the line number in the code, which helps find it.

    Strings/values are optional, and can be a list or a single element.
    If provided, they must both have the same number of elements.
    """

    import six
    import inspect

    # Sanity check: If strings/values are provided, they have to be the same number of elements
    if strings:
        if type(strings) is list:
            string_list= strings
        elif isinstance(strings, int):
            string_list= [strings]
        elif isinstance(strings, six.string_types):  # six works with both Pythons 2 & 3
            string_list= [strings]
        else:
            print("WARNING! whocalledme:1462: Don't know how to deal with type %s" % type(strings) )
            return
    else:
        string_list= []
    ...
    calling_python= os.path.splitext( os.path.basename(inspect.stack()[1].filename) )[0]
    calling_function= inspect.stack()[1].function
    if strings or values:
        if len(string_list) != len(variable_list):
            print(f"UH OH! whocalledme: Passed different number of strings ({len(string_list)}) and values ({len(variable_list)})")
            return
        else:
            # Loop through variables
            for curr_string, curr_value in zip(string_list, variable_list):
                print(f"{calling_python}:{num2print}:\t{calling_function}() :\t{curr_string}= '{curr_value}'")
    # No strings or values
    else:
        print(f"{calling_python}:{num2print}:\t{calling_function}() called by {inspect.stack()[2].function}()")
