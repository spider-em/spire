...
def filenames2numbers(flist):
    ...
    nums = re.compile(r'\d+\D')  # ints followed by one non-int char
    nlist = []
    for file in flist:
        m = nums.findall(file)
        if len(m) == 1:
...
class askWindow:
    def __init__(self, n, startVar, endVar, win=None):
        ...
        tkinter.Label(f1, text="There are %d images" % n).pack(side="top", fill='x', pady=2)
        self.rb = tkinter.Radiobutton(f1, text="Display all of them", value="all",
                         variable=self.displayVar, command=self.setall)
        self.rb.pack(side='top', anchor='w', pady=4)
        g = Pmw.Group(f1, tag_pyclass = tkinter.Radiobutton,
        ...
        ok = tkinter.Button(f2, text="OK", command=self.setvars)
        ok.pack(side='left', padx=4, pady=4)
        quit = tkinter.Button(f2, text="Cancel", command=self.cancel)
...
def getImageRange(n, size, top=None, interactive=1):
    ...
    if hasgui:
        ...
        win = tkinter.Toplevel(top)
        win.title("specify image series")
        askWindow(n, startVar, endVar, win=win)  #### previously assigned to 'aw'
        win.wait_window(win)
        start = startVar.get()
        end = endVar.get()
        return (start,end)
...
def loadImageSeries(filelist=None, parent=None, byte=1, istart=None, iend=-1,
    ...
    info = Spiderutils.spiderInfo(file)
    if info:
        dtype, size = info[0], info[1]
    else:
        dtype = 'non-spider'
        try:
            im = Image.open(file)
    ...
    if dtype == "volume":
        ...
        hdr = Spiderutils.getSpiderHeader(filelist[0])
        Nimages = int(hdr[26])
    else:
        Nimages = Nfiles
    if DISPLAY_ALL == 1:
    ...
    # ----------------------------------------------
    if dtype == "image" or dtype == "non-spider":
    ...
    # ----------------------------------------------
        ...
        im = Image.open(file)
        ###print(f"im: {type(im)}: {hasattr(im, 'nimages')}, {hasattr(im, '_nimages')}")

        # if a selection doc provided
        if selectdoc:
            ###print(os.path.basename(__file__),":295:selectdoc:",selectdoc)
            try:
                D = Spiderutils.readdoc(selectdoc, keys='all')
            except Exception as e:
                print(f"\nERROR!! {e}")
                print(  f"  Couldn't open '{selectdoc}'")
                print(   "  Exiting...\n")
                exit()

            ###print(os.path.basename(__file__),":297:D:",len(D), D)
            for key in D.keys():
                ##Spiderutils.whocalledme(300, key, D[key][0])
                ##print(os.path.basename(__file__),":299:",key, D[key][0])
                imgnum= int(D[key][0]) - 1
                im.seek(imgnum)
                im.info['filename'] = "%s@%d" % (basename[:-4], imgnum+1)
                if byte:
            ...
            return imglist
        # if a range of image numbers
        else:
            ###print(os.path.basename(__file__),":299: no selectdoc")
            ...
            ##print(f"{os.path.basename(__file__)}.loadImageSeries:311: im: {vars(im)}")
            ###print(f"{os.path.basename(__file__)}:312: SpiderImagePlugin: {SpiderImagePlugin.__file__}")

            for i in range(s1,s2):   #ni):
                im.info['filename'] = "%s@%d" % (basename[:-4], i+1)
                ##print(f"{os.path.basename(__file__)}.loadImageSeries:316: {i}: {im.info['filename']}")
                ##print(f"{os.path.basename(__file__)}.loadImageSeries:317: im: {vars(im)}")
                im.seek(i)
                ###print(f"{os.path.basename(__file__)}.loadImageSeries:319: im: {vars(im)}")
                if byte:
                    imglist.append(im.convert2byte())
                else:
                    imglist.append(im)
            #try:
                ...
                #for i in range(s1,s2):   #ni):
                    ...
                    ##im.info['filename'] = str(i+1)
                        ...
                        #imglist.append(im)
            #except:
            #print(f"{os.path.basename(__file__)}:306: Error trying to load SPIDER stack: {file}")
            #return []

    # ----------------------------------------------
    elif dtype == "volume":
        
        V = Spiderarray.spider2array(file)  # V is a 3D numpy array
        N = V[istart-1:iend]
        if byte:
            V = N
