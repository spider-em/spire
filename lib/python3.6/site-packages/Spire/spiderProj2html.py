...
def proj2html():
    ...
    # first open a window to get filename (and Notes) from user.
    # When window closes, it calls saveProj2html
    Proj2htmlForm(title="Save project as html pages",
                         subtitle=lbltxt)  #, subtitlepos='w')
    # (previously assigned to 'sp2h')


class Proj2htmlForm(spiderClasses.scrolledForm):
    ...
    def mkMainFrame(self):
        ...
        browse = tkinter.Button(fr2,  text="Browse",
                        command=spiderUtils.Command( spiderGUtils.getFile, self.ef,"*.html") )
    ...
    def go(self):
        ...
        notes = self.st.get()
        if filename == "":
            spiderGUtils.displayError("an output filename is required")
            return
        self.top.destroy()
        saveProj2html(filename, notes)  #### previously assigned to 'sph'
...
class saveProj2html:
    ...
    def __init__(self, mainfile=None, notes=None):
        ...
        try:
            ...
            spiderGUtils.displayError("Unable to open %s for writing." % mainfile)
            return
        base, ext = os.path.splitext(mainfile)
        htmldir = base + "_files"
        mkdir = spiderUtils.makeDirectory(htmldir)
        if not mkdir:
            spiderGUtils.displayError("Unable to create directory." % htmldir)
            return
    ...
    def make_main_page(self):
        ...
        if hasattr(GB.P, 'pixelsize') and GB.P.pixelsize != "":
            H.append( ['pixel size: ', spiderUtils.trim_zeroes(GB.P.pixelsize)] )
        if hasattr(GB.P, 'kv') and GB.P.kv != "":
            H.append( ['electron energy: ', spiderUtils.trim_zeroes(GB.P.kv)] )
    ...
    def make_bat_page(self, id, filename, procedure):
        ...
        bp = spiderUtils.stringify(bparms)
        if bp != None and bp != "":
