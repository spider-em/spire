    ...
    for win in winlist:
        if win.winfo_class() == 'Toplevel':
            if title == win.title():
                win.lift()
                return 0
    w = tkinter.Toplevel(GG.topwindow)
    w.title(title)
    # set up icon
    icon = tkinter.Toplevel(w)
    tkinter.Label(icon, image=GG.icon).pack()
    w.iconwindow(icon)
...
def askOKorCancel(message, title=None, parent=None):
    ...
    if spiderUtils.isMainThread():
        try:
            if parent != None:
                ret = messagebox.askokcancel(title, message, parent=parent)
            else:
        ...
        ret = spiderUtils.threadedGUIcall()
...
def askYesorNo(message, title=None, parent=None):
    ...
    if parent != None:
        ret = messagebox.askyesno(title, message, parent=parent)
    else:
...
def askSaveAsFilename(filetypes=None, initialfile=None):
    ...
    if initialfile == None:
        f = filedialog.asksaveasfilename(filetypes=ft)
    else:
        f = filedialog.asksaveasfilename(filetypes=ft, initialfile=initialfile)
    if type(f) == type(()):
        return ""
    else:
        #f = f.replace("/tmp_mnt","")
...
# askdirectory stuff for Python3
class Chooser(commondialog.Dialog):
...
def askDirectory():
    directory = filedialog.askdirectory(initialdir=os.getcwd(),mustexist=1)
    return directory
...
def getDir2(entry, parent=None):
    directory = filedialog.askdirectory(initialdir=os.getcwd(),mustexist=1, parent=parent)
...
def getFiles(parent=None, filetypes=None, entry=None):
    ...
    if filetypes != None:
        ft.append( (filetypes,filetypes )) # "*.dat" --> ("*.dat", "*.dat")
    ft.append(("All files", "*"))
    f = filedialog.askopenfilename(parent=parent, multiple=1, filetypes=ft)
...
class addBatchWindow:
    def __init__(self,filename=None, dialog=None, label=None):
        ...
        # -------------------------------
        top = tkinter.Frame(self.w, background=GG.bgd01, relief=GG.frelief,
                    borderwidth=GG.brdr)
        top.pack(side='top', fill='x', expand=1)
        tkinter.Label(top,text="Add a batch file to a dialog",
              background=GG.bgd01).pack()

        # -------------------------------
        mid = tkinter.Frame(self.w)
        ...
        browse = tkinter.Button(mid, text="Browse",
                        command=spiderUtils.Command(getFile,self.batfile))
        browse.pack(side='right', padx=4,pady=4)
        
        # -------------------------------
        bot = tkinter.Frame(self.w, background=GG.bgd01, relief=GG.frelief,
                    borderwidth=GG.brdr)
        bot.pack(side='bottom', fill='x', expand=1)
        ok = tkinter.Button(bot, text='Ok', command=self.addBatch)
        quit = tkinter.Button(bot, text='Cancel', command=self.w.destroy)
...
def addBatch(filename=None, dialog=None, label=None):
    import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:506:\t{inspect.stack()[0][3]}() called by {inspect.stack()[2][3]}()")
    addBatchWindow(filename,dialog,label)  #### has been previously assigned to abwin
...
class textWindow:
    def __init__(self, title=None, font=None, toptext=None, importfile=None):
        ...
        if toptext != None:  # add a label to the top
            self.toptext = tkinter.Label(self.w, text=toptext)
    ...
    def write(self, string, tag=None):
        ...
        if tag == None:
            self.textbox.component('text').insert(tkinter.END, str(string)+'\n')
        else:
            self.textbox.component('text').insert(tkinter.END, str(string)+'\n', tag)
    ...
    def clear(self):
        if self.hasTextbox(): self.textbox.delete("1.0",tkinter.END)
...
class editWindow:
    def __init__(self, file=None, title=None, font='output'):
        ...
        # buttons at bottom
        fb = tkinter.Frame(self.w, relief=GG.frelief, borderwidth=GG.brdr, background=GG.bgd01)
        fb.pack(side='bottom', fill='x', expand=1)
        if file != None:
            self.filename = file
            save = tkinter.Button(fb, text='Save', command=self.save)
            save.pack(side='left', padx=4, pady=4)
        saveas = tkinter.Button(fb, text='Save as', command=self.saveas)
        saveas.pack(side='left', padx=4, pady=4)
        quit = tkinter.Button(fb, text='Quit', command=self.quit)
        quit.pack(side='right', padx=4, pady=4)
...
def textBoxInsert(textbox, text):
    textbox.component('text').insert(tkinter.END, str(text)+'\n')
...
def viewFile(filename=None, type=None, verbose=0):  #0):
    ...
    if not os.path.exists(filename):
        filename = spiderUtils.truepath(filename)
        if not os.path.exists(filename):
    ...
    if type != None:
        if type == 'text':
            ...
            textWindow(title=fn, font='output', importfile=filename)  #### previously assigned to tw
        elif type == 'edit':
            fileEdit(filename)
        elif type == 'image':
        ...
        if extnodot == GB.P.dataext:
            ...
            spiderUtils.executeCommand(app, [filename])
        else:
...
re_comment =  re.compile(r'; +/ *\w*')   # looks for patterns: " ; / comment"
...
def getParms():
    ...
    B = spiderUtils.fileReadLines(parmfile)
    ...
    while ptr < n:
        ...
        if line[0] == ';':
            ...
            if re_comment.match(line):
                ...
                try:
                    key = int(line[0])
                    int(line[1])   # previously assigned to 'iii'
                    float(line[2])   # previously assigned to 'val'
                except:
                    ptr = ptr + 1
                    continue
            ...
            try:
                key = int(s[0])
                int(s[1])   # previously assigned to 'iii'
                float(s[2])   # previously assigned to 'val'
                if line.find(';') > -1:
                    ss = line.split(';',1)
                    comment = ss[1]
                else:
...
def makeVBOFFwindow():
    ...
    tkinter.Radiobutton(w, text='Change to VB ON', value=1,
                variable=var).pack(anchor='w')
    tkinter.Radiobutton(w, text='Proceed with VB OFF, dont save files to project',
                value=2, variable=var).pack(anchor='w')
    tkinter.Radiobutton(w, text='Cancel operation', value=3,
                variable=var).pack(anchor='w')
...
def writeUserPrefs(verbose=0):
    ...
    if GG.prefs.useLocalPrefsFile and spiderUtils.hasProject():
        local = 1
    ...
    for k in keys:
        if k == 'colors':
            ...
            value = prefdict[k]
            if not local and spiderUtils.hasProject():    # remove extension
    ...
    ###print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:835:writeUserPrefs:\tGG.interface.file={GG.interface.file}")
    if spiderUtils.fileWriteLines(prefsFile, outlines) and verbose:
...
def readUserPrefs(verbose=1):
    ...
    ###import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:845:\t{inspect.stack()[0][3]}() called by {inspect.stack()[1][3]}()")
    filename = GG.interface.file
    ###print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:920:readUserPrefs:\tfilename={filename}")
    if not os.path.exists(filename):
        writeUserPrefs(0)
    else:
        B = spiderUtils.fileReadLines(filename)
        if B == None:
        ...
        for key in keys:
            ...
            if len(value) > 0 and value[0] == '{' and value[-1] == '}':
                ...
                if spiderUtils.hasProject() and key in ['filenumFile', 'paramFile']:
        ...
        ###print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:916:readUserPrefs:\tGG.interface.file={GG.interface.file}")
        if GG.prefs.useLocalPrefsFile == 0 or not spiderUtils.hasProject():
            GG.interface.file = os.path.join(os.environ['HOME'],".spire")
        else:
            GG.interface.file = os.path.join(GB.P.projdir, '.spire')
        ###print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:921:readUserPrefs:\tGG.interface.file={GG.interface.file}")
...
def XXXwriteUserPrefs(verbose=0):
    ...
    try:
        ...
        messagebox.showerror('File Save Error', e)
...
def XXXreadUserPrefs(verbose=1):
    ...
    if not os.path.exists(filename):
        ...
        try:
            f = open(filename, 'r')
            GG.interface = pickle.load(f)
            f.close()
            if verbose:
                GB.outstream("settings loaded from %s" % filename)
        except IOError as e:
            messagebox.showerror('File Open Error', e)
            return
        # shortcuts
...
def fontSize(font=None):
    ...
    tkfont = tkinter.font.Font(font=font)
    tkinter.Label(GG.topwindow, text='test',font=tkfont)  #### previously assigned to 'test'
