                ...
                if os.path.exists(batchpath):  # found the source
                    ...
                    if write_all == 0:
                        if os.path.exists(targetfile):
                            ...
                            overwriteDialog(targetfile, w, ovar, batchpath)
...
def overwriteDialog(filename, w, var, src):
    w.title('Warning')
    f = tkinter.Frame(w, background = GG.bgd01, relief=GG.frelief, borderwidth=GG.brdr)
    ovtxt = f'{filename} \nalready exists. Overwrite with\n{src}?'
    tkinter.Label(f, background = GG.bgd01, text=ovtxt, justify='right').pack(side='left', padx=5, pady=5)
    ...
    w.bind('<Return>', lambda e, w=w: parmquit(w))
