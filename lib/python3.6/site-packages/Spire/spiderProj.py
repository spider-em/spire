...
from . import GB,GG
from . import spiderUtils  #### from   .spiderUtils import *
from . import spiderGUtils  #### from   .spiderGUtils import *
from . import spiderParam
from . import spiderConfig
from . import spiderDButils
from . import spiderFilenums
from . import spiderClasses
from . import spiderFtypes
from . import spiderAddbatch
######################################################################
#
# PROJECT METHODS
#
...
class rshTest(spiderClasses.threadObject):
    ...
    def run(self):
        ...
        out = spiderAddbatch.getoutput(cmd)
...
def askLocalorRemote(projecthost, localhost) :
    var = tkinter.IntVar()
    var.set(0)
    w = spiderGUtils.newWindow('Select Local or Remote host')
    f1 = tkinter.Frame(w, background=GG.bgd01, relief=GG.frelief, borderwidth=GG.brdr)
    ...
    lbl= tkinter.Label(f1, text=txt, background=GG.bgd01, anchor=tkinter.W, justify='left')
    ...
    tkinter.Radiobutton(w, text=remote_txt, value=1,variable=var).pack(anchor='w')
    tkinter.Radiobutton(w, text=local_txt, value=2, variable=var).pack(anchor='w')
    tkinter.Radiobutton(w, text='Cancel', value=3,variable=var).pack(anchor='w')
...
def makeProjectTable():
    ...
    if not spiderUtils.hasProject(): return

    T = {}

    keys = GB.P.RunList  # RunList is a list if run IDs
    ###print("spiderProj:128:makeProjectTable:\tGB.P.RunList:", GB.P.RunList)
    keys.sort()
    for key in keys:
        br = spiderUtils.getBatchRun(key)
        ###print(f"spiderProj:132:makeProjectTable:\tkey={key}, br={br}")
        if br != 0:
            #if isinstance(br, spiderClasses.SpiderBatchRun):  fails
            ###print(f"spiderProj:135:makeProjectTable:\tisBatchRun(br)={isBatchRun(br)}")
            if isBatchRun(br):
                T[key] = br
                #printBatchRun(br)
            #else:
...
def addhost(event,ehost,entdir):
    dir = entdir.get()
    if dir != "":
        host = spiderUtils.getHost(dir)
        if host != "":
            ehost.setentry(host)

def projEdit():
    if spiderUtils.hasProject():
        projForm('edit')
    else:
        projOpen()
...
def getProjectDatabase(projfile):
    ...
    if not spiderUtils.hasDatabase():
...
class projForm:
    ...
    def __init__(self, mode='new', pid=""):
        ...
        w = spiderGUtils.newWindow(title)
        if w == 0: return
        self.w = w
        
        self.mode = mode        
        if pid == "" and spiderUtils.hasProject('ID'): pid = GB.P.ID
        self.pid = pid
        
        # check the current directory for any config files
        lc = spiderUtils.dirlist('.', "*.xml")
        localConfigs = []
        for cfg in lc:
        ...
        if mode == 'edit':
            ...
            name = ext = ""
            file = 'proj001.dat'  # the default project name in the GUI
            host = spiderUtils.gethostname()
            dir = spiderUtils.getCurrentDir()
            ...
            cfgs = spiderUtils.dirlist(GG.sysprefs.configdir, "*.xml")
            self.configlist = []
            for cfg in cfgs:
        ...
        cwd = spiderUtils.getCurrentDir()

        # see if the database/projectid frame is required
        fr0 = tkinter.Frame(w, relief=GG.frelief, borderwidth=GG.brdr, background=GG.bgd01)
        fr1 = tkinter.Frame(w, relief=GG.frelief, borderwidth=GG.brdr)
        fr2 = tkinter.Frame(w, relief=GG.frelief, borderwidth=GG.brdr)
        fr3 = tkinter.Frame(w, relief=GG.frelief, borderwidth=GG.brdr)
        bot = tkinter.Frame(w, relief=GG.frelief, borderwidth=GG.brdr, background=GG.bgd01)
        px = 10
        py = 4
        ...
        self.usedatabase = 1
        if spiderUtils.hasConfig('useDatabase'):
            self.usedatabase = GB.C.useDatabase
        if GB.extDatabase and self.usedatabase != 0:
            dblbl = tkinter.Label(fr0, text="Get project info from database:", background=GG.bgd01)
            fetch = tkinter.Button(fr0, text='Get', command=self.getDatabaseInfo)
        ...
        browseButton = tkinter.Button(fr2, text="Browse",
                          command=spiderUtils.Command(spiderGUtils.getDir2,self.edir,w))
        ...
        configButton = tkinter.Button(fr3, text="Browse",
                          command=spiderUtils.Command(spiderGUtils.getFile,self.ecfg.component('entryfield'),"*.xml"))
        configButton.pack(side='right', padx=px, pady=5)
        self.ecfg.pack(anchor='w', fill='x', expand=1, padx=px, pady=py)
        spiderGUtils.seeEntry(self.ecfg)
            
        self.edir.pack(anchor='w', fill='x', expand=1, padx=px, pady=py)
        spiderGUtils.seeEntry(self.edir)

        self.loadvar = tkinter.IntVar()
        ...
        w.bind('<Return>', self.readEntries)
        ...
        # Button frame
        okButton = tkinter.Button(bot, text = "OK",
                          command=self.readEntries)
        quitButton = tkinter.Button(bot, text = "Cancel",
                            command=w.destroy) 
    ...
    def getDatabaseInfo(self):
        ...
        if id == "" or not id.isdigit():
            spiderGUtils.displayError('Please enter a valid Project ID number')
            self.w.lift()
            return
    ...
    def readEntries(self, event=None):
        # 1) check entries for legal values
        ProjID = self.epid.get().strip()
        name = self.ename.get().strip()

        # ensure that extension is '.dat', since dbm will enforce it anyway
        projfile = os.path.splitext(self.efile.get().strip() )[0] + '.dat'
        ...
        if name == '' or projfile == '' or ext == '' or projdir == '':
            spiderGUtils.displayError('Please fill in all the blanks','Error')
            self.w.lift()
            return
        elif len(ext) != 3:
            spiderGUtils.displayError('File extension must\nbe 3 letters long','Error')
            self.w.lift()
            return

        # 2) create the project directory, if it doesn't exists
        if not os.path.exists(projdir):
            if spiderGUtils.askOKorCancel("Create %s?" % (projdir),projdir + " does not exist"):
                if not spiderUtils.makeDirectory(projdir):
                    spiderGUtils.displayError('Unable to access ' + projdir,'Error')
                    self.w.lift()
                    return
            else:
                return
            
        spiderGUtils.killWindow(self.w)

        if not spiderUtils.hasProject():
            GB.P = spiderClasses.SpiderProject(id=ProjID)
            
        projdir = spiderUtils.truepath(projdir)
        projfile = spiderUtils.truepath(projfile)
        projdir, projext = os.path.splitext(projdir)  # no extension on project file
        config = spiderUtils.truepath(config)
        spiderUtils.chdir(projdir)
        ...
        if config != "":
            if not os.path.exists(config):
                ...
                if not os.path.exists(config):
                    spiderGUtils.displayError("Unable to find %s" % config)
        ...
        projSet()
        spiderUtils.projSave()
        
        # 8) Parameter file:
        #    First, see if config specifies 'no parameters'
        if spiderUtils.hasConfig('useParameterFile') and GB.C.useParameterFile != 0:
            pass
        else:
            return
...
def loadProject():
    if not spiderUtils.hasProject():
        spiderGUtils.displayError('No project found')
        return 0
    if not spiderUtils.hasConfig():
        spiderGUtils.displayError('Project does not have configuration loaded')
        return 0
    
    projdir = GB.P.projdir
    #print "loadProject: %s" % projdir
    if spiderUtils.makeDirectory(projdir):
        spiderUtils.chdir(projdir)
    else:
        return 0
    
    loadBatch()  #### previously assigned to 'x'
    return 1
               
# returns 1 if all subdirectories found in pdir, 0 otherwise
def checkProj(dirlist, pdir=None):
    #print "checkProj"
    all_present = 1
    for d in dirlist:
        ##dir = d[0]
...
def projSet():
    ...
    if spiderUtils.hasConfig('Title'):
        GG.spidui.bigLabel.configure(text=GB.C.Title)
    if spiderUtils.hasConfig('Image'):
        GG.spidui.putImage(GB.C.Image)

    if GG.sysprefs.filenumFile.strip() != "":
        GG.sysprefs.filenumFile = spiderUtils.addExtension(GG.sysprefs.filenumFile, GB.P.dataext)
    #print GG.sysprefs.filenumFile
    
    # check for local parameter file first
    localparam = os.path.split(spiderUtils.addExtension(GG.sysprefs.paramFile,GB.P.dataext))[1]
    currentparam = os.path.join(GB.P.projdir,localparam)
    if os.path.exists(currentparam):
        GG.sysprefs.paramFile = currentparam
    else :
        GG.sysprefs.paramFile = spiderUtils.addExtension(GG.sysprefs.paramFile,GB.P.dataext)

    # figure out if they're running on the localhost
    projhost = GB.P.host
    localhost = spiderUtils.getHost()
    ...
    spiderUtils.chdir(GB.P.projdir)
    
    if GG.prefs.savelogfile:
        logfilename = os.path.join( GB.P.projdir, spiderUtils.newfilename(GG.sysprefs.logfile) )
    ...
    ###print(f"spiderProj:641:projSet:\tlist(GB.P.T.keys()):", list(GB.P.T.keys()) )
    return 1
        
#----------------------------------------------------------------    
def projOpen(filename=None):
    ###import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:646:\t{inspect.stack()[0][3]}() called by {inspect.stack()[1][3]}()")
    if filename == None:
        filename = spiderGUtils.askfilename()
    if filename == "":
        return
    ...
    if os.sep in filename:
        ...
        if not spiderUtils.chdir(projpath):
    ...
    ###print("spiderProj:projOpen:660:filename:", filename, GB.DB)
    ...
    ###print("spiderProj:projOpen:668:project:", project)
    ...
    if project:
        ...
        if not spiderUtils.remoteProjectPath(GB.P.projdir, projpath):
            GB.errstream("unable to find project directory")
            return
        
        ###print("spiderProj:690:projOpen: GB.DB.RunList:", GB.P.RunList)
        ...
        spiderGUtils.displayError(filename + " doesn't appear to contain a valid project.")
        return
    ...
    GB.outstream(prjtxt)
    spiderGUtils.getParms()   # load GB.Parameters
    
    if spiderUtils.hasProject() and spiderUtils.hasConfig():
...
def goprojSave():
    if spiderUtils.hasProject():
        ...
        spiderGUtils.displayError('No project to save')
...
def mkprojdirs(dirobj):
    ...
    if ddir != '.' and not os.path.exists(ddir):
        if not spiderUtils.makeDirectory(ddir):
            GB.errstream("unable to create %s" % ddir)
            return

    # create any subdirectories
    cwd = os.getcwd()
    spiderUtils.chdir(ddir)
    for item in items:
        if type(item) == type(["list"]):
            mkprojdirs(item)
    spiderUtils.chdir(cwd)

def loadBatch():
    "copy batch files from src locations to target directories"
    if not spiderUtils.hasProject():
        spiderGUtils.displayError('spiderProj.loadBatch: No project found')
        return 0
    if not spiderUtils.hasConfig():
        spiderGUtils.displayError('spiderProj.loadBatch: No configuration loaded')
        return 0
    ...
    for loc in GB.C.path:
        if os.path.exists(loc):
            ...
            cant_find.append(loc)
    if len(srcdirs) == 0:
        GB.errstream("WARNING: Unable to find source directory for batch files")
        GB.errstream(f"  {cant_find}")
        return 0
    #print "load batch, srcdirs : %s" % str(srcdirs)

    # create subdirectories under project directory
    if not spiderUtils.chdir(GB.P.projdir):
            ...
            if os.path.exists(sdir):
                ...
                if os.path.exists(batchpath):  # found the source
                    if not os.path.exists(targetdir):
                        if not spiderUtils.makeDirectory(targetdir):
                            continue
                    ...
                    if write_all == 0:
                        if os.path.exists(targetfile):
                            w = spiderGUtils.newWindow('Overwrite?')
                    ...
                    # copy the file
                    if spiderUtils.checkFileAccess(targetdir):
                        shutil.copy(batchpath,targetdir)
...
def overwriteDialog(filename, w, var):
    w.title('Warning')
    f = tkinter.Frame(w, background = GG.bgd01, relief=GG.frelief, borderwidth=GG.brdr)
    ovtxt = filename + ' already exists. Overwrite?'
    tkinter.Label(f, background = GG.bgd01, text=ovtxt).pack(side='left', padx=5, pady=5)
    f.pack(side='top', fill='both', expand=1)
    
    tkinter.Radiobutton(w, text='Yes', value=1,
                variable=var).pack(anchor='w')
    tkinter.Radiobutton(w, text='No, do not overwrite', value=2,
                variable=var).pack(anchor='w')
    tkinter.Radiobutton(w, text='Overwrite all', value=3,
                variable=var).pack(anchor='w')
    tkinter.Radiobutton(w, text="Copy all new files, but don't overwrite old", value=4,
                variable=var).pack(anchor='w')
    tkinter.Radiobutton(w, text='Cancel batch file copying', value=5,
                variable=var).pack(anchor='w')
    var.set(1)
    fb = tkinter.Frame(w, background = GG.bgd01, relief=GG.frelief, borderwidth=GG.brdr)
    b = tkinter.Button(fb, text='Ok', command=w.destroy)
