...
class DialogWindow(spiderClasses.scrolledForm):
    """ Dialog used to launch programs """
    
    def processargs(self):
        ###keywords = list(self.kwargs.keys())
        self.dialog = self.kwargs["dialog"]
        self.progstring = tkinter.StringVar()
    ...
    def helpme(self):
        spiderGUtils.spiderHelp(self.dialog.help)
    ...
    def mkMainFrame(self):
        ...
        for item in dialog.cmdlist:
            if isButton(item):
                ...
                frg.grid(row=k, column=0, columnspan=3, sticky=tkinter.NSEW, padx=5, pady=5)
                fg = frg.interior()
    ...
    def addProgButton(self, frame, button, row):
        ...
        cmdlist = button[2]
        # the Label
        tkinter.Label(frame, text=label).grid(row=row, column=0, sticky=tkinter.E, padx=5, pady=5)
        # the Run button
        b = tkinter.Button(frame, text=btxt, width=self.bwidth, activebackground=execGreen,
                   command=spiderUtils.Command(self.runBatch,cmdlist))
        b.grid(row=row, column=1, sticky=tkinter.E, padx=5, pady=5)
        # the Edit button
        if self.isProg(cmdlist[0]):
            ...
            e = tkinter.Button(frame, text='Edit')
            e.configure(command=spiderUtils.Command(self.editProg,progstr,cmdlist,b,e))
            self.helpMessage(e,"Edit " + progstr)
        else:
            " regular Spider batch file "
            progstr = self.checkString(cmdlist)
            batstr = self.no_args(progstr)
            e = tkinter.Button(frame, text='Edit', command=spiderUtils.Command(self.editBatch,cmdlist))
            self.helpMessage(e,"Display " + batstr + " in a batch form")
        e.grid(row=row, column=2, sticky=tkinter.E, padx=5, pady=5)
    ...
    def editProg(self, progstr, cmdlist, runbutton, editbutton):
        ...
        w = spiderGUtils.newWindow("command line")
        if w == 0: return

        self.progstring.set(progstr)
        slen = len(progstr)
        tkinter.Entry(w, textvariable=self.progstring, width=slen).pack(side='top', fill='x', expand=1)  #### previously assigned to 'entry'
        tkinter.Button(w, text='ok', command=w.destroy).pack(padx=2, pady=2)  #### previously assigned to 'ok'
        w.wait_window(w)
        ...
        runbutton.configure(command=spiderUtils.Command(self.runBatch, cmdlist))
        self.helpMessage(runbutton, "Run " + progstr)
        editbutton.configure(command=spiderUtils.Command(self.editProg, progstr, cmdlist, runbutton, editbutton))
...
class DialogEditForm:
    ...
    def __init__(self, w, dialog=None):
        ...
        ftop = tkinter.Frame(w, background = GG.bgd01, relief=GG.frelief, borderwidth=GG.brdr)
        nlabel = tkinter.Label(ftop, background = GG.bgd01, text= '(short) Name: ')
        en = tkinter.StringVar()
        en.set(self.dialog.name)
        ename = tkinter.Entry(ftop,textvariable=en)

        tlabel = tkinter.Label(ftop, background = GG.bgd01, text= 'Title: ')
        et = tkinter.StringVar()
        et.set(self.dialog.title)
        etitle = tkinter.Entry(ftop,textvariable=et)
        
        ed = tkinter.StringVar()
        ed.set(self.dialog.dir)
        dirwidth = 32
        edir = tkinter.Entry(ftop,textvariable=ed, width=dirwidth)
        spiderGUtils.seeEntry(edir)
        dlabel = tkinter.Button(ftop, text="Directory:", background = GG.bgd01,
                        command=spiderUtils.Command(spiderGUtils.getDir,ed,edir))
        
        eh = tkinter.StringVar()
        eh.set(self.dialog.help)
        ehelp = tkinter.Entry(ftop,textvariable=eh)
        spiderGUtils.seeEntry(ehelp)
        hlabel = tkinter.Button(ftop, text="Help URL:", background = GG.bgd01,
                        command=spiderUtils.Command(spiderGUtils.getFile,eh))
        ...
        # ----- bottom frame ----
        fbot = tkinter.Frame(w, background = GG.bgd01, relief=GG.frelief, borderwidth=GG.brdr)
        okbut = tkinter.Button(fbot, text="OK", command=self.okDone)
        addbut = tkinter.Button(fbot, text=" Add button",
                        command=spiderUtils.Command(self.addButton))
        delbut = tkinter.Button(fbot, text="Delete",
                        command=spiderUtils.Command(self.delButton))
        prebut = tkinter.Button(fbot, text="Preview",
                        command=self.previewDialog)
        quitbut = tkinter.Button(fbot, text='Cancel',command=self.quit)
    ...
    def delButton(self):
        ...
        if not found:
            spiderGUtils.displayError('Nothing selected')
        else:
    ...
    def okDone(self):
        ...
        if missingtxt != "":
            spiderGUtils.displayError("Please fill in the\n'%s' field" % missingtxt)
        else:
            self.result = "ok"
            self.quit()
    ...
    def previewDialog(self):
        ...
        w = spiderGUtils.windowExists(tmpdialog.name)
        if w != 0:
            spiderGUtils.killWindow(w)
    ...
    def getValues(self, dialog=None):
        ...
        if dialog == None:
            dialog = self.dialog
        dialog.name  = self.entries[0].get().strip()
        dialog.title = self.entries[1].get().strip()
        dialog.dir   = self.entries[2].get().strip()
        dialog.help  = self.entries[3].get().strip()
    ...
    def quit(self):
        ...
        w = spiderGUtils.windowExists(self.dialog.name)
        if w != 0:
            spiderGUtils.killWindow(w)
...
class ButtonFrame:
    ...
    def __init__(self, btn, parent):
        if len(btn) < 3: return
        self.type = "button"
        self.frame = tkinter.Frame(parent, relief='groove', borderwidth=2)
        fr = self.frame

        self.selected = 0
        self.label = tkinter.StringVar()
        self.btntxt = tkinter.StringVar()
        self.progs = tkinter.StringVar()

        self.label.set(btn[0])
        self.btntxt.set(btn[1])
        self.progs.set(spiderUtils.stringify(btn[2]))

        l1 = tkinter.Label(fr,text="Label: ")
        l1.grid(row=0,column=0, sticky='w')
        tkinter.Entry(fr, textvariable=self.label).grid(row=0,column=1, sticky=tkinter.EW)
        l2 = tkinter.Label(fr,text="Button text: ")
        l2.grid(row=1,column=0, sticky='w')
        tkinter.Entry(fr, textvariable=self.btntxt).grid(row=1,column=1, sticky=tkinter.EW)
        l3 = tkinter.Label(fr,text="Program(s): ")
        l3.grid(row=2,column=0, sticky='w')
        tkinter.Entry(fr, textvariable=self.progs).grid(row=2,column=1, sticky=tkinter.EW)
...
class GroupFrame:
    ...
    def __init__(self, group, parent):
        ...
        btnlist = group[1]
        bgd = GG.bgd02
        self.frame = tkinter.Frame(parent, relief='groove', borderwidth=4,
                           background=bgd)
...
def editDialog(dialogname=None):
    ...
    if not spiderUtils.hasConfig():
        GB.C = spiderClasses.Config()
        
    if dialogname != None and dialogname in GB.C.Dialogs:
        dialog = GB.C.Dialogs[dialogname]
    else:
        dialog = Dialog()
        
    w = spiderGUtils.newWindow("Edit Dialog")
    if w == 0: return
