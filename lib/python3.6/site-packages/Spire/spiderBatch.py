...
###from subprocess import getoutput  # NOT USED?

from . import GB, GG
from . import spiderUtils  #### from .spiderUtils import *
from . import spiderGUtils  #### from .spiderGUtils import *
from . import spiderExecwin
from . import spiderFilenums  #### from .spiderFilenums import *
from . import spiderResult
from . import spiderRemote
...
def priorRun(batdir, batfile):
    ...
    for id in GB.P.RunList:
        run = spiderUtils.getBatchRun(id, outputs=1)
        if run == 0: continue
        dir = run.dir
        file = run.batfile
        #print dir, file
        if dir == batdir and file == batfile:
            ...
            f1 = spiderFilenums.range2list(fn)
            fn = spiderFilenums.getFileNumbers()
            if fn != None and fn != "":
                f2 = spiderFilenums.range2list(fn)
                for item in f2:
                    if item in f1:
                        return 1
    return 0
...
def updateFileOutputs(idnew, withoutputs=1):
    ###print("spiderBatch:92: updateFileOutputs")
    if len(GB.P.RunList) < 2: return
    newfiles = []
    newrun = spiderUtils.getBatchRun(idnew, outputs=1)
        ...
        for item in self.runarg:
            ...
            for batfile in batlist:
                ...
                if batfile.find("<prog>") > -1:
                    prog = spiderParam.replaceTagwithParam(batfile)
                ...
                if GG.sysprefs.checkPrior:
                    if priorRun(batdir, batfile):
                        if GG.prefs.delAsk:
                            ...
                            if not spiderGUtils.askOKorCancel(msgtxt, 'Warning'):
                                stop = 1
                                break
                        self.prior_run = 1
...
def makeSpiderCommand(batfile, batargs=None, spider=None, dataext=None):
    ...
    if not batargs:
        ...
        if batargs[-1] == '&':
            batargs = batargs[:-1]
        args = spiderUtils.stringify(batargs, sep=' ')
...
def execBatch(batdir, batfile):
    ...
    if not spiderUtils.chdir(execdir):
        print("execBatch: unable to chdir")
        return -1

    if not os.path.exists(batfile):
        spiderGUtils.displayError("Unable to find " + batfile)
        return -1
    
    # Save the original batch text
    originalBatch = spiderUtils.fileReadLines(batfile)   #savecopy(batfile)
    ...
    filenumsFilename = spiderFilenums.getFilenumFilename(os.path.join(execdir,batfile))
    ###import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:313:\t{inspect.stack()[0][3]}() : filenumsFilename = '{filenumsFilename}'")

    if filenumsFilename != "":
        useFileNums = 1
    else:
        useFileNums = 0
    ###print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:319:\t{inspect.stack()[0][3]}() : useFileNums = '{useFileNums}'")
    newfilenums = None
    filenumbers = None
    
    if useFileNums:
        # If preference is to use fileunums entry...
        if GG.prefs.useFilenumsEntry:
            # use values in the File Numbers entry box and temp filenum file
            filenumbers = spiderFilenums.getFileNumbersEntry() # check the entry box
            ###print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:328:\t{inspect.stack()[0][3]}() : filenumbers = '{filenumbers}'")
                
            if filenumbers == None or filenumbers == "":
                if not os.path.exists(filenumsFilename):
                    txt = "No file numbers have been specified.\nContinue anyway?"
                    if not spiderGUtils.askOKorCancel(txt,"No file numbers!"):
                        return -1
            else:
                newfilenums = spiderUtils.uniqueFilename(prefix='fn', extension=GB.P.dataext)
                ###print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:337:\t{inspect.stack()[0][3]}() : newfilenums = '{newfilenums}'")
                filenumbers = spiderFilenums.readFileNumbers(filenumFile=newfilenums)  # writes entrybox to file
                ###print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:339:\t{inspect.stack()[0][3]}() : filenumbers = '{filenumbers}'")
                br.filenumfile = newfilenums
                br.usingtmpfilenums = 1

                # change the filenumbers file in the batch file to tmp name
                ret = spiderReplace.batReplaceFilenums( batfile, filenumFile=os.path.splitext(newfilenums)[0] )
                if ret == 0:
                    GB.errstream("spiderBatch: unable to replace filenums file\n")
        # If not using filenums entry...
        else:
            ...
            if not os.path.exists(filenumsFilename):
                ...
                if not spiderGUtils.askOKorCancel(txt,"No file numbers!"):
                    return -1
            #filenumbers = spiderFilenums.readFileNumbers()  # writes to default filenums file
            
    cmd = makeSpiderCommand(batfile, batargs)
    ###print("spiderBatch:348: cmd:", cmd)
    GB.outstream( "\nSending command: %s \n" % cmd)
    ...
    # subprocess has a different syntax than popen2
    #exe= cmd.split()[0]
    #args= ' '.join(cmd.split()[1:])
    #print("spiderBatch:363: cmd:", cmd)
    ##print("spiderBatch:364: args:", args)
    ###child= subprocess.Popen([exe, args], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    child= subprocess.Popen(cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    # without text=True, bytes would be returned, which would then need to be decoded
    ...
    spiderUtils.makeNonBlocking(outfd)
    spiderUtils.makeNonBlocking(errfd)
    outdata = errdata = ""
    outeof = erreof = 0
    ...
    while 1:
        ...
        if outfd in ready[0]:
            outchunk = child.stdout.read()
            if outchunk == '': outeof = 1
            outdata += outchunk
            if not gotresult:
                reshdr = reshdr + outchunk
        if errfd in ready[0]:
            errchunk = child.stderr.read()
        ...
        if not gotresult and outdata.find("Results file") > -1:
            ...
            if results == "":
                spiderGUtils.displayError('Results file not found in output')
                spiderExecwin.execBatchCleanup(ew, success=0)
                spiderUtils.fileWriteLines(batfile, originalBatch)
                return -1
            ...
            spireout = spiderUtils.results2spireout(results)
    ...
    err = child.wait()
    if err != 0:
        print(f"spiderBatch: Popen.child returned with error={err}")
        return -1
    ...
    if stopline != "" and stopline.find(" NORMAL ") > -1: # success        
        ...
        #                  ========================
        b = spiderClasses.SpiderBatchRun()
        # set default values
        b.id = spiderUtils.makeRunID(br.time_start)
        b.batfile = batfile
        b.dir = batdir
        ...
        B = spiderUtils.fileReadLines(batfile)
        b.text = B
        b.filenumbers = ""
        b.parameters = spiderGUtils.getParms()
        ...
        spiderUtils.saveBatchRun(b)  #### previously assigned to 'bat_id'
        ...
        #print('spiderBatch:579:execBatch: windowExists("Project Table")', windowExists("Project Table"))
        #print('spiderBatch:580:execBatch: projectTable', hasattr(GG,'projectTable'))
        #print('spiderBatch:581:execBatch: windowExists("Project Viewer")', windowExists("Project Viewer"))
        if spiderGUtils.windowExists("Project Table") and hasattr(GG,'projectTable'):
...
def readStdoutFile(spdfile):
    ...
    if os.path.exists(spdfile):
        s = spiderUtils.fileReadLines(spdfile)
        for line in s:
            d = line.replace("\012", "\n")
            E.append(d)
    return E 
