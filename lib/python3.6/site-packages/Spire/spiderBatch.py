    ...
    def run(self):
        ...
        for item in self.runarg:
            ...
            for batfile in batlist:
                ...
                ###import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:192:{inspect.stack()[0][3]}\tid= '{id}'")
...
def execBatch(batdir, batfile):
    ...
    # stdout and stderr are bytes, and need to be decoded below
    # In Python 3.7+, it can be decoded here with 'text=True'
    # In Python <=3.6, the corresponding keyword is 'universal_newlines=True'
    # Rather than mess with the Python version, I'll simply decode them as needed for now.
    ...
    while 1:
        ...
        if outfd in ready[0]:
            outchunk = child.stdout.read().decode()
            if outchunk == '': outeof = 1
            outdata += outchunk
            if not gotresult:
                reshdr = reshdr + outchunk
        if errfd in ready[0]:
            errchunk = child.stderr.read().decode()
    ...
    ## Better to report error below, such that ExecWin will automatically close
    #import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:461:{inspect.stack()[0][3]}\terr= '{err}'")
    #if err != 0:
        #GB.outstream(f"\nspiderBatch: command '{cmd}' returned with error={err}", 'red')
        #if errdata : GB.outstream(f"  {errdata}", 'red')
        #return -1
    ...
    #
        ...
        GB.errstream(f"Error in SPIDER run: {batfile}", 'red')
        GB.errstream(f"  Command '{cmd}' returned with error={err}", 'red')
        GG.spidui.write_queue('GG.topwindow.bell()')
        GB.errstream(f"  Results file: {br.resultfile}", 'red')
        if errdata : GB.errstream(f"  {errdata}", 'red')
