...
def headSpireout(spireout):
    spireparms = spiderClasses.SpireoutParms()  # create an object to hold results
    done = 0
    ntimes = 20
    nbytes = 280
    ...
    for i in range(ntimes):  # try reading the file up 20 times (until get pid)
        try:
            ...
            if len(s) == 0:
                continue
            ss = s.split('\n')
            ###print("spiderResult:headSpireout:48: ss", ss)
            for line in ss:
                if line.find("Running") > -1:
                    runline = line.replace("Running:", "").strip()
                    ###print("spiderResult:headSpireout:52: runline", runline)
                    ...
                    t = line.split()
                    ###print("spiderResult:headSpireout:65: DATE", t)
            ...
            if not os.path.exists(spireout):
                GB.outstream(f'{spireout} does not exist\n')
            else:
...
def tailSpireout(spireout):
    if not os.path.exists(spireout):
        GB.errstream( "tailSpireout: cannot find %s" % os.path.abspath(spireout) )
        return ()
    cmd = 'tail -5 ' + spireout
    out = spiderUtils.getCommandOutput2(cmd)
    if len(out) < 1:
...
def docData(dlist):
    ...
    for d in dlist:
        ...
        if ext == GB.P.dataext:
            ...
            lastkey = None
            cmd = 'tail ' + filename
            tt = spiderUtils.getCommandOutput2(cmd) # tail of doc file
            a = tt.split('\n')
            a.reverse()
            for i in a:
                if i != "" and i[1] != ";":
...
re_tsfile = re.compile(r"WRITTEN +\d\d-[A-Z][A-Z][A-Z]-\d\d\d\d at \d\d:\d\d:\d\d")
...
def newOutputs(filename):
    ...
    if size < MAXSIZE:
        ...
        while done == 0:    # loop over the chunks
            ...
            if FirstTime == 1:
                FirstTime = 0
                getBatchData(B)  #### previously assigned to 'N'
...
if __name__ == '__main__':
    ...
    if len(sys.argv[1:]) > 0:
        br = spiderUtils.readResults(sys.argv[1])
