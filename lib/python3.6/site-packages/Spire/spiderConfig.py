...
from . import spiderFtypes  #### from .spiderFtypes import isSpiderBatchfile, isSpiderProcedurefile
import tkinter
import Pmw
...
def mkNewConfig():
    if not spiderUtils.hasProject():
        spiderGUtils.noProjectError()
        return
    newConfigForm(wintitle="Spire Configuration",
                        title="Configuration",
                        subtitle="Fill in the blanks, values can be saved as an XML file",
                        config=GB.C)
    # (previously assigned to 'ncf')
...
def dir2dialog(d):
    ...
    for item in dlist:
        if spiderFtypes.isSpiderBatchfile(item):
...
def isBatchFile(f):
    if spiderFtypes.isSpiderBatchfile(f) or spiderFtypes.isSpiderProcedurefile(f):
        return True
    else:
        return False
    
def getDirFiles(dirname):
    if spiderUtils.dirsizeTooBig(dirname):
        return [dirname, []]
...
def readDirsFromDisk(projectDir=None, tmpfile="tmp999.xml"):
    ...
    spiderUtils.fileWriteLines(tmpfile, xmltext)
    spiderGUtils.fileEdit(tmpfile)
    

def mkconfig(src, ftypes, xmlout):
    " make a Spire config (not an xml config) "
    if not os.path.exists(src):
        spiderGUtils.displayError("Unable to find %s" % src)
        return
    ...
    C.path = [src]
    dlist = spiderUtils.read_dirtree(src, ftypes)
    C.Dirs = dlist[1]
    ...
    #if not spiderUtils.chdir(projectdir):
        #return
    filelist = dlist[1]
    dialoglist = []
    for item in filelist:
        if type(item) == type(["list"]):
            dirname = item[0]
            ###dirlist = item[1]
            if not spiderUtils.chdir(dirname):
...
class newConfigForm(spiderClasses.scrolledForm):
    ...
    def mkMainFrame(self):
        ...
        lgrp = lg.interior()
        lglabel = tkinter.Label(lgrp, text="Indicate the source location of batch files")
        lglabel.pack(side='top', anchor='w')
        srcdir = tkinter.Entry(lgrp, textvariable=self.pathVar)
        lgbut = tkinter.Button(lgrp, text="Directory: ",
                       command=spiderUtils.Command(spiderGUtils.getDir,self.pathVar,srcdir))
        ...
        dgrp = dg.interior()
        dgbframe = tkinter.Frame(dgrp)
        dgbframe.pack(side='left')
        addb = tkinter.Button(dgbframe, text="new dialog",
                      command = spiderUtils.Command(self.editdialog,'new'))
        addb.pack(side='top', padx=px, pady=py)
        editb = tkinter.Button(dgbframe, text="edit dialog", command=self.editdialog)
        editb.pack(side="top", padx=px, pady=py)

        self.box = Pmw.ScrolledListBox(dgrp, listbox_selectmode=tkinter.SINGLE,
        ...
        ggrp = gg.interior()
        ggbframe = tkinter.Frame(ggrp)
        ggbframe.pack(side='left')
        daddb = tkinter.Button(ggbframe, text="new directory",
                       command=spiderUtils.Command(self.editdir, 'new'))
        daddb.pack(side='top', padx=px, pady=py)
        deditb = tkinter.Button(ggbframe, text="edit directory", command=self.editdir)
        deditb.pack(side='top', padx=px, pady=py)
        treeb = tkinter.Button(ggbframe, text="read directory", command=self.readDirtree)
        treeb.pack(side='top', padx=px, pady=py)

        self.dirbox = Pmw.ScrolledListBox(ggrp, listbox_selectmode=tkinter.SINGLE,
                                      items=self.dirlist,
                                      listbox_height=5, vscrollmode='static',
    ...
    def setValues(self):
        ...
        if hasattr(cfg, 'Dialogs'):
            if 'dialogList' in cfg.Dialogs:
                ...
                self.dialoglist = list(cfg.Dialogs.keys())
    ...
    def saveas(self, askload=1):
        ...
        if hasattr(GB.P, 'config'):
            default = GB.P.config
        filename = spiderGUtils.askSaveAsFilename(filetypes="*.xml", initialfile=default)
        if filename == "": return

        setconfig = 'False'
        if askload:
            msg = "Load %s into Spire?" % os.path.basename(filename)
            setconfig = spiderGUtils.askYesorNo(msg, title="set new configuration")
    ...
    def viewconfig(self):
        ...
        spiderGUtils.fileEdit(filename)
    ...
    def editdialog(self, mode='edit'):
        ...
        w = spiderGUtils.newWindow(wintitle)
        if w == 0: return
        ...
        if df.result == "ok" and dialog != None:
            ...
            if mode == 'new':
                ...
                if dialogname not in b:
                    self.box.component('listbox').insert(tkinter.END, dialogname)
                    self.box.component('listbox').see(tkinter.END)

    def readDirtree(self):
        directory = spiderGUtils.askDirectory()
        #print directory
        if len(directory) > 0:
            tree = spiderUtils.read_dirtree(directory)
            t = tree[1]
            dirlist = []
            for item in t:
                if type(item) == type(["list"]):
    ...
    def editdir(self, mode='edit'):
        ...
        dirnameVar = tkinter.Stringvar()
        dircontentsVar = tkinter.Stringvar()
        ...
        b = self.dirbox.get()
        if dn not in b:
            self.dirbox.component('listbox').insert(tkinter.END, dn)
            self.dirbox.component('listbox').see(tkinter.END)
    ...
    def dirWindow(self, w, nameVar, contentsVar):
        ...
        dirent = tkinter.Entry(f1, textvariable=nameVar)
        dirbut = tkinter.Button(f1, text="Directory\n name  ",
                        command=spiderUtils.Command(spiderGUtils.getDir,nameVar,dirent))
        dirbut.pack(side='left', padx=4, pady=4)
        dirent.pack(side='right', fill='x', expand=1)
        f2 = tkinter.Frame(w)
        f2.pack(side='top', fill='both', expand=1)
        dcnent = tkinter.Entry(f2, textvariable=contentsVar)
        dcnbut = tkinter.Button(f2, text="Directory\ncontents",
                        command=spiderUtils.Command(spiderGUtils.getFiles,None,None,contentsVar))
        ...
        ok = tkinter.Button(fb, text="Ok", command=w.destroy)
        ok.pack(side='left', padx=4, pady=4)
        done = tkinter.Button(fb, text="Cancel", command=spiderUtils.Command(self.dirdone,w))
...
def editConfig(filename=None):
    ...
    res = spiderGUtils.askOKorCancel(txt, title='Edit config file')
    if res:
        spiderGUtils.fileEdit(filename)
...
def readXMLConfig(XMLfile=None, setproject=1):
    ...
    try:
        ...
        spiderGUtils.displayError("unable to parse %s" % (XMLfile))
        return 0
...
def prettyxml(file=None):
    ...
    tw = spiderGUtils.textWindow(title="Configuration view", font='output')
    ...
    for line in p:
        if line.count('<') == 2 and line.count('>') == 2:
            a = line.find('>')  # end of 1st tag
            s = line[:a+1]   # up to the first tag (with indent)
            line = line[a+1:]
            b = line.find('<')  # start of 2nd tag
            content = line[:b].strip()
...
#
def saveConfig():
    if not spiderUtils.hasConfig(): return
...
class saveXMLConfig:
    ...
    def writeButton(self,button, indent=0):
        ...
        proclist = button[2]
        self.writetag('proc', spiderUtils.stringify(proclist), indent=indent+2)
        self.writeln(sp + "</button>")
