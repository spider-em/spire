...
from . import spiderFonts
from . import spiderPrefs
from . import spiderSysprefs
from . import spiderInspect
from . import spiderExtapps
...
class optionNotebook:
    ...
    def __init__(self, page=""):
        if not spiderUtils.hasProject():
            ...
            return
        w = spiderGUtils.newWindow("Options")
        if not w: return
        ...
        b = tkinter.Button(fb, text="Done", command=self.quit)
    ...
    def makeConfigurationPage(self):
        ...
        b1 = tkinter.Button(fd, text = 'Edit', command=self.editDialog)
        b1.pack(side='left', padx=10, pady=10)
        if GG.prefs.balloonHelp:
            self.balloon.bind(b1,'Edit the selected Dialog')
            
        b2 = tkinter.Button(fd, text = 'New', command=self.newDialog)
        ...
        #group.component('tag').configure(background=GG.bgd01)
        fdir = group.interior()
        fd = tkinter.Frame(fdir)
        b1 = tkinter.Button(fd, text = 'View directories', command=spiderGUtils.treeview)
        b1.pack(side='left', padx=10, pady=15)
        if GG.prefs.balloonHelp:
            self.balloon.bind(b1,'Tree view of files')
        b2 = tkinter.Button(fd, text = 'Read from disk',
    ...
    def makeInspectPage(self):
        ...
        button = tkinter.Button(f1, text='view project',
                        command=spiderInspect.inspectProject)
        button.grid(row=0,column=0, padx=10,pady=10)
        button = tkinter.Button(f1, text='view GB',
                        command=lambda g=GB: spiderInspect.inspectObject(g))
        button.grid(row=0,column=1, padx=10,pady=10)
        button = tkinter.Button(f1, text='view preferences',
                        command=lambda g=GG.prefs: spiderInspect.inspectObject(g))
        button.grid(row=0,column=2, padx=10,pady=10)
        button = tkinter.Button(f1, text='project file',
                        command=spiderInspect.inspectProjectFile)
        button.grid(row=1,column=0, padx=10,pady=10)
        f1.pack(side='top', fill='both', expand=1)
        f2 = tkinter.Frame(page)
        self.object = tkinter.StringVar()
        e = tkinter.Entry(f2, textvariable= self.object)
        e.pack(side='left',fill='x',padx=10,pady=10)
        e.bind('<Return>', self.viewObject)
        b = tkinter.Button(f2, text="View it", command=self.viewObject)
    ...
    def makeFontsPage(self):
        ...
        spiderFonts.FontWindow(group.interior())  #### previously assigned to 'fw'
        group.pack(fill = 'both', expand = 1, padx = 10, pady = 10)
    ...
    def updateListbox(self):
        if spiderUtils.hasConfig():
            ...
            self.cb.component('listbox').delete(0,tkinter.END)
            self.cb.clear()
            for item in self.dialogmenu:
                self.cb.component('listbox').insert(tkinter.END, item)
            self.cb.selectitem(0)

    def getFilename(self):
        spiderGUtils.getFile(self.cfile,"*.xml")
        self.readConfig()
    ...
    def saveAsConfig(self):
        file = spiderGUtils.askSaveAsFilename("*.xml")
        if file == "":
            return
        GG.sysprefs.configFile = file
        self.cfile.set(file)
