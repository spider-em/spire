...
from . import GB
from . import GG
from . import spiderBatch
from . import spiderBatform
###from . import spiderClasses
from . import spiderConfig
from . import spiderDialog
from .spiderExecwin import execWindow  # needed for some reason in threadedGUIexec()
from . import spiderFilenums
from . import spiderProj
from . import spiderProj2html
from . import spiderParam
from . import spiderUtils  #### from   .spiderUtils import *
from . import spiderGUtils  #### from   .spiderGUtils import *
from . import spiderOptions
from . import spiderView
from . import spiderSequence
from . import spiderIcons
###from . import spiderReplace
from . import spiderImage
...
def threadedGUIexec(threadname):
    ...
    if func not in globs:
        ...
        if hasattr(tpkg, 'module'):
            # try importing module.func?
            module = tpkg.module
            if module in globs:
                ###print("  threadedGUIexec: module %s found" % module)
                func = module + "." + func
            else:
            ...
            return
        ###import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:73:\t{inspect.stack()[0][3]}() called by {inspect.stack()[1][3]}()") ; exit()
    kwargs = tpkg.kwargs
    
    result = eval(func)(*(), **kwargs)  # call the function 
...
def about():
    w = spiderUtils.newWindow('About SPIRE')
    if w == 0: return
    appname = "%s Version %s\n\n" % (GG.applicationname, GG.applicationversion)
    copyright = 'Copyright 2006, 2010 Health Research Inc. , Menands, NY,\nAlbany, NY.'
    copyright = copyright + ' All rights reserved.'
    tkinter.Label(w, text=appname + copyright).pack(side='top', padx=10, pady=10)
    tkinter.Button(w, text='Ok', command=w.destroy).pack(side='top', padx=10, pady=10)
...
class SpiderGUI:
    ...
    def __init__(self, master, project=0, usesmallfont=0):
        ...
        self.icon = tkinter.Toplevel(self.top)
        tkinter.Label(self.icon, image=GG.icon).pack()
    ...
    def setup(self):
        if self.usesmallfont:
            ...
            GG.pady=3
        self.readUserPrefs()
        GG.icon = tkinter.PhotoImage(data = spiderIcons.spider) #file = iconimage)
        # set up output streams for GUI
        self.msgfr = tkinter.Frame(GG.topwindow,
                           relief=GG.frelief,
                           borderwidth=GG.brdr,
                           background=GG.bgd01)
    ...
    def createMenus(self, master): # master = toplevel window
        ...
        # menu bar ----------------------------------------------
        self.mBar = tkinter.Frame(master, relief=GG.frelief, borderwidth=GG.brdr,
                          background=GG.sysbgd)

        self.makeProjMenu(master)
        self.makeParmMenu(master)
        self.makeCommMenu(master)
        if spiderUtils.hasConfig():
    ...
    def makeProjMenu(self, master):
        ...
        Proj_button = tkinter.Menubutton(self.mBar, text='Project', underline=0,
                                 relief=self.brelief)
        Proj_button.pack(side='left', padx=5, pady=5)
        Proj_button.menu = tkinter.Menu(Proj_button, tearoff=0)

        Proj_button.menu.add_command(label='New', underline=0,
                                     command=spiderProj.projNew)
                                     #command=spiderUtils.Command(spiderProj.projForm,'new'))
        ...
        Proj_button.menu.add_command(label='Edit', underline=0,
                                     command=spiderUtils.Command(spiderProj.projEdit))
    ...
    def testfunction(self):
        spiderGUtils.showUserPrefs()

    def makeParmMenu(self, master):
        Parm_button = tkinter.Menubutton(self.mBar, text='Parameters', underline=0,
                                 relief= self.brelief)
        Parm_button.pack(side='left', padx=5, pady=5)
        Parm_button.menu = tkinter.Menu(Parm_button, tearoff=0)
    ...
    def makeCommMenu(self, master):
        Comm_button = tkinter.Menubutton(self.mBar, text='Commands', underline=0,
                                 relief= self.brelief)
        Comm_button.pack(side='left', padx=5, pady=5)
        Comm_button.menu = tkinter.Menu(Comm_button, tearoff=0)
        ...
        Comm_button.menu.add_command(label='Configuration', underline=0,
                                     command=spiderUtils.Command(spiderOptions.optionNotebook,'Configuration'))
    ...
    def makeDialogMenu(self, master=None):
        ...
        Proc_button = tkinter.Menubutton(self.mBar, text='Dialogs', underline=0,
                                 relief= self.brelief)
        Proc_button.pack(side='left', padx=5, pady=5)
        Proc_button.menu = tkinter.Menu(Proc_button, tearoff=0)
        ...
        for d in GB.C.Dialogs['dialogList']:
            ...
            Proc_button.menu.add_command(label=d, underline=0,
                                     command=spiderUtils.Command(spiderDialog.makeDialogWindow,dialog))
    ...
    def makeBatchMenu(self, master=None):
        ...
        Prog_button = tkinter.Menubutton(self.mBar, text='Batch files', underline=0,
                                 relief= self.brelief)
        Prog_button.pack(side='left', padx=5, pady=5)
        Prog_button.menu = tkinter.Menu(Prog_button, tearoff=0)
        self.PB = Prog_button
        # make the submenus
        ...
        for d in dirlist:
            if len(GB.M[d]) == 0:  # no procedures
                continue
            progs1 = tkinter.Menu(Prog_button.menu, tearoff=0)
    ...
    def addProgToProgMenu(self, dir, progmenu):
        for batfile in GB.M[dir]:
            ...
            progmenu.doit = tkinter.Menu(progmenu, tearoff=0)
            progmenu.doit.add_command(label='Edit',
                                      command=spiderUtils.Command(spiderBatform.getBatch,
                                                      fullname))
            progmenu.doit.add_command(label='Run',
                                command=spiderUtils.Command(spiderBatch.runbatch,runarg))
    ...
    def makeSequenceMenu(self, master=None):
        ...
        Seq_button = tkinter.Menubutton(self.mBar, text='Sequences', underline=0,
                                 relief= self.brelief)
        Seq_button.pack(side='left', padx=5, pady=5)
        Seq_button.menu = tkinter.Menu(Seq_button, tearoff=0)
    ...
    def makeHelpMenu(self):
        ...
        Help_button.menu.add_command(label='Project Help', underline=0,
                                     command=spiderUtils.Command(self.callSpireHelp,'project') )

        Help_button.menu.add_command(label='Spire Help', underline=0,
                                     command=spiderUtils.Command(self.callSpireHelp, 'spire') )
    ...
    def createHeader(self,master):
        bgd = GG.bgd01
        self.fr_hdr = tkinter.Frame(master)
        self.fr_img  = tkinter.Frame(self.fr_hdr, relief=GG.frelief,
                             borderwidth=GG.brdr, background=bgd)
        if os.path.exists(GG.mainImage):
            self.img = tkinter.PhotoImage(file=GG.mainImage)  #, palette=256)   #/256/256)
            self.imglabel = tkinter.Label(self.fr_img, image=self.img)
            self.imglabel.pack(padx=10, pady=10, fill='both')
        else:
            print("cannot find " + GG.mainImage)
        
        self.frlabel = tkinter.Frame(self.fr_hdr, relief=GG.frelief,
                             borderwidth=GG.brdr, background=bgd)
        self.labeltxt = GG.bigLabeltxt
        self.bigLabel = tkinter.Label(self.frlabel, text=self.labeltxt, background=bgd,
                           font= GG.spid.font['large'])
        self.projLabel = tkinter.Label(self.frlabel, text='Project: ',background=bgd)
        self.dirLabel = tkinter.Label(self.frlabel, text='Directory: ',background=bgd)
    ...
    def putImage(self, imagefile):
        ...
        self.img = tkinter.PhotoImage(file=imagefile)  #, palette=256)   #/256/256)
    ...
    def createFileNumbers(self, master):
        ...
        frfilenums = tkinter.Frame(self.top, relief=GG.frelief, borderwidth=GG.brdr,
                           background=GG.bgd01)
        #fnlabel = tkinter.Label(frfilenums, text="File numbers:", background=GG.bgd01)
        #fnlabel.pack(side='left', padx=5, pady=5)
        fnbtn = tkinter.Button(frfilenums, text="File numbers:", background=GG.bgd01,
                       command=spiderFilenums.FileNumbersButton)
        fnbtn.pack(side='left', padx=5, pady=5)
        GG.balloon.bind(fnbtn, "select file numbers")
        fnentry = tkinter.Entry(frfilenums, width=24, textvariable=GG.disp_fnums)
        fnentry.bind('<Return>', lambda e : spiderFilenums.readFileNumbers(e))
    ...
    def callSpireHelp(self, which='spire'):
        if which == 'spire':
            spiderGUtils.spiderHelp(url = GG.SpireHelpURL)
        elif which == 'project':
            spiderGUtils.spiderHelp(url = GG.sysprefs.helpURL)  # read from XML file?

    def readUserPrefs(self):
        spiderGUtils.readUserPrefs(verbose=0)
