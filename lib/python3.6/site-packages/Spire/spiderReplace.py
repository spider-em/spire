...
def batReplaceFilenums(batfile, filenumFile=None):
    ##import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:29:\t{inspect.stack()[0][3]}() called by {inspect.stack()[1][3]}()")
    ##print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:30:\t{inspect.stack()[0][3]}() : filenumFile = {filenumFile}")

    if not os.access(batfile, os.W_OK):
        try:
            os.chmod(batfile,0o664)
        except:
    ...
    os.remove(batfile)
    if spiderUtils.fileWriteLines(batfile, B):
        return 1
    else:
        return 0
...
def vboffdialog():
    if spiderUtils.isMainThread():
        res = spiderGUtils.makeVBOFFwindow()
    else:
        res = spiderUtils.threadedGUIcall()
...
def replaceFilenums(B, newfilenumfile=None):
    ...
    ##import inspect
    ##print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:173:\t{inspect.stack()[0][3]}() called by {inspect.stack()[1][3]}()")
    if newfilenumfile == None:
        newfilenumfile = spiderUtils.uniqueFilename(prefix='fn')
        
    ###print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:177:\t{inspect.stack()[0][3]}() newfilenumfile : '{newfilenumfile}'")

    k = 0
    for line in B:
        if line.find(GB.end_of_header) > -1:
            break
        nline = spiderUtils.stripComment(line)
        ...
        if nline.find(GG.sysprefs.filenumSymbol) == 0:
            ...
            a = line.find(';')
            if a > -1:
                symbol,value = spiderUtils.spiderSymbol(line[:a])
                comment = line[a:]
            ...
            # if no comment
                ...
                symbol,value = spiderUtils.spiderSymbol(spiderUtils.stripComment(line))
                newline = symbol + newfilenumfile + '     ; file numbers doc file'
                
            ###print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:200:\t{inspect.stack()[0][3]}() newline : '{newline}'")
            B[k] = newline
            break
        else:
            k +=1
...
def end2en(B):
    ...
    k = 0
    for line in B:
        nline = spiderUtils.stripComment(line).upper()
