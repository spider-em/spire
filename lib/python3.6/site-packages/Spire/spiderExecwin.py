...
def execBatchCleanup(ew=None, success=1):
    ...
    if ew != None:
        if hasattr(ew.br,'usingtmpfilenums') and ew.br.usingtmpfilenums != 0:
            ...
            spiderUtils.delete(ew.br.filenumfile)
        ...
        if not GG.prefs.saveResults:
            if os.path.exists(ew.br.resultfile):
                ...
                spiderUtils.delete(ew.br.resultfile)
        # remove remote shell files
        if hasattr(ew.br, 'script'):
            spiderUtils.delete(ew.br.script)
        if hasattr(ew.br, 'spdout'):
            spiderUtils.delete(ew.br.spdout)
        if hasattr(ew.br, 'spderr'):
            spiderUtils.delete(ew.br.spderr)
        ew.quit()
##################################################################
...
class execWindow:
    ...
    def __init__(self, execdir, cmd, br):
        ###import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:125:\t{inspect.stack()[0][3]}() called by {inspect.stack()[1][3]}()")
        self.br = br
        self.top = tkinter.Toplevel(GG.topwindow)
        ...
        self.fr = tkinter.Frame(self.top, background=GG.bgd01)

        self.cmdlabel = tkinter.Label(self.fr, text=dispcmd, background=cmd_color1,
                        font=GG.spid.font['small'], relief=tkinter.RIDGE, borderwidth=3)
        dirlabel = tkinter.Label(self.fr, text="Dir: " + execdir, font=GG.spid.font['small'],
                         background=GG.bgd01)
        hstlabel = tkinter.Label(self.fr, text="Host: " + GB.P.host,
                         font=GG.spid.font['small'],background=GG.bgd01)
        self.pidlabel = tkinter.Label(self.fr, text="PID:  ", font=GG.spid.font['small'],
                         background=GG.bgd01)
        self.reslabel = tkinter.Label(self.fr, text="Results file: ",
                         font=GG.spid.font['small'],background=GG.bgd01)
        ...
        resbutton= tkinter.Button(self.fr, text='results file',
                          command=self.showResults)      
        resbutton.pack(side='left', padx=5, pady=5)
        stopbutton= tkinter.Button(self.fr, text='Kill', activebackground='red',
    ...
    def showResults(self):
        ...
        out =  spiderUtils.getCommandOutput(cmd)
        output = out[0].split("\n")

        if not hasattr(self,'tw'):
            # first time called
            self.tw = spiderGUtils.textWindow(title=results, font='output', toptext=rstr)
    ...
    def execExit(self):      # called by the STOP button
        ...
        res = spiderGUtils.askYesorNo(msg, title="Kill SPIDER?")
        if not res:
            return
        host = self.br.host
        self.br.kill = 1
        ...
        if self.pid.isdigit():
            ...
            process = subprocess.Popen(killcmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)  #, text=True)
            ...
            stdout, stderr = process.communicate()  # adapted from https://stackoverflow.com/a/31867499
            if process.returncode:
                GB.outstream(f"\nUnable to kill process : {stderr.decode()}", 'red')
            else:
