...
def get_Cs_kv_pixsize():
    ...
    cs = kv = pixsize = ""
    if spiderUtils.hasProject() and hasattr(GB.P, 'parameters'):
        ...
        for k in keys:
            ...
            if comment.find('spherical aberration') > -1:
                cs = spiderUtils.trim_zeroes(d[1])
            elif comment.find('electron energy') > -1:
                kv = spiderUtils.trim_zeroes(d[1])
            elif comment.find('pixel size') > -1 or comment.find('pixelsize') > -1:
                pixsize = spiderUtils.trim_zeroes(d[1])
...
def readParmFile(filename=None, window=None):
    ...
    if parmdict == None or len(parmdict) == 0:
        spiderGUtils.displayError("Unable to get parameters from %s" % filename, 'file read error')
        return
...
def presentParmForm(parmdict=None):
    if parmdict != None:
        GB.P.parameters = parmdict
    parmForm(wintitle='SPIDER Parameters',
                  title='Project Parameters')
    # (previously assigned to 'pf')

def saveParmdict2File(parmdict=None, filename=None):
    if parmdict == None:
        parmdict = GB.P.parameters
    if filename == None:
        filename = spiderGUtils.askSaveAsFilename(initialfile=GG.sysprefs.paramFile)
        if filename == "":
            return
        #print "saveParmdict2File", filename
        #print "saveParmdict2File", GG.sysprefs.paramFile
        
    hdr = spiderUtils.makeDocfileHeader(filename, batext='spi')
    filep = open(filename, 'w')
    filep.write(hdr)
    keys = list(parmdict.keys())
    keys.sort()
    for k in keys:
        parm = parmdict[k]
        key = int(parm[0])
...
def setProjectParms():
    ...
    keys = list(GB.P.parameters.keys())
    for k in keys:
...
def askParmNew(outputfile):
    win = spiderGUtils.newWindow('Read parameters from a file')
    f1 = tkinter.Frame(win, background=GG.bgd01, relief=GG.frelief, borderwidth=GG.brdr)
    ...
    f2 = tkinter.Frame(win, relief=GG.frelief, borderwidth=GG.brdr)
    ok = tkinter.Button(f2, text='OK', command=lambda w=win: readParmFile(window=w))
    ok.pack(side='left', padx=3, pady=3)
    cancel = tkinter.Button(f2, text='cancel', command=win.destroy)
...
def parmNew(defaultfile=None):
    ...
    if not hasattr(GB,'P'):
        spiderGUtils.displayError('Please open a project.', 'New Project')
        return
...
def getProjDir():
    ...
    if dir == "" or os.path.exists(dir) != 1:
        ...
        spiderGUtils.displayError('Error',askstr)
    return dir


def parmOpen():
    if not spiderUtils.hasProject():
        spiderGUtils.displayError('Please open a project.')
        return
    dir = getProjDir()
    if dir == "" or spiderUtils.chdir(dir) == 0: return
    ...
    if not os.path.exists(parmfile):
        ...
        if spiderGUtils.askOKorCancel(askstr,'Error'):
            parmForm(wintitle='SPIDER Parameters',
                          title='Project Parameters', new=1)
            # (previously assigned to 'pf')
        return
    else:
        if spiderFtypes.istextfile(parmfile) and spiderFtypes.isSpiderDocfile(parmfile):
            pass
        else:
            spiderGUtils.displayError('%s\n does not appear to be a SPIDER doc file' % parmfile)
            return
    ...
    parmForm(wintitle='SPIDER Parameters',
                  title='Project Parameters',
                  subtitle=spiderUtils.projFilename(GG.sysprefs.paramFile), new=0)
    # (previously assigned to 'pf')
...
def pixsizeAutoBindings(event=None,entries=None):
    ...
    if pixelsize:
        ...
        try:
            ...
            spiderGUtils.displayError("Error computing pixels size.\nPlease enter a valid pixel size", "Error")
            #print p
...
def kvAutoBindings(event=None,entries=None):
    ...
    try:
        ...
        spiderGUtils.displayError("Please enter a valid value for electron energy", "Error")

###################################################################
#

class parmForm(spiderClasses.scrolledForm):
    
    def makeButtons(self, bot):
        ##print("spiderParam:431:makeButtons: bot:\t\t", bot, type(bot) )
        ##print("spiderParam:432:makeButtons: bot.master:\t", bot.master, type(bot.master) )
        self.addButton(bot, text='OK', command=self.parmSave)
        self.addButton(bot, text='Cancel', command=self.quit, side='right')
        bot.master.bind('<Return>', self.parmSave)
    ...
    def mkMainFrame(self):
        ...
        if spiderUtils.hasProject() and GB.P.parameters:
            parmdict = GB.P.parameters
        else:
            parmdict = GB.DP
        ...
        for key in keys:
            ...
            if name.find('pixel size') > -1:
                ...
                ###focusentry = F
                if spiderUtils.hasProject():   # see if it got pixsize from database
    ...
    def parmSave(self, event=None):
        ###print("spiderParam:529: parmSave")
