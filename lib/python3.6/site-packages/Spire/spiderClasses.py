...
class SpiderProject:
    ...
    def __init__(self, **kwargs):
        ...
        self.projfile = "proj001"  # I don't know when this value is used
        self.projdir = os.getcwd()
        self.dataext = "dat"
        self.host = spiderUtils.gethostname()
        self.RunList = []   # a list of batchrun id's
        self.SpiderBatchDir = ""
        self.config = GG.sysprefs.configFile
        (date, time, x) = spiderUtils.nowisthetime()
...
class ProjectDatabase:
    ...
    def __init__(self, filename, new=0, quiet=0):
        ...
        if new:
            ###print(f"spiderClasses:141: new={new}, __init__ calling openDatabase")
        ...
        # else open pre-existing project file
            ...
            ###print(f"spiderClasses:151: new={new}, __init__ calling openDatabase")
            try:
    ...
    def get(self, key='Project'):
        ...
        if self.openDatabase():
            ...
            print("spiderClasses:187: openDatabase returned: 0")
            return None

    def put(self, key, object):
        " for putting Project, Config in database "
        pc = ['Project', 'Config']
        #import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:191:\t{inspect.stack()[0][3]}() called by {inspect.stack()[1][3]}()")
        #print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:192:{inspect.stack()[0][3]}\tobject: '{object}', dir: {dir(object)}")
        #print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:193:{inspect.stack()[0][3]}\tself.database: '{self.database}', keys: {self.Keys()}")
        if key not in pc:
            GB.errstream('Project file key error: %s must be from %s' % (key, self.Keys() ) )
            return
    ...
    def addBatchRun(self, batchrun=None, id=None):
        ###print("spiderClasses:218: addBatchRun calling openDatabase")
        if self.openDatabase():
            if id == None:
                id = str(batchrun.id)
    ...
    def delBatchRun(self, id):
        ###print("spiderClasses:228: delBatchRun calling openDatabase")
        if self.openDatabase():
            if id in self.database:
    ...
    def RunList(self):
        rl = []
        ###print("spiderClasses:238: RunList calling openDatabase")
        if self.openDatabase():
            keys = list(self.database.keys())
            self.closeDatabase()
            for k in keys:
    ...
    def openDatabase(self, blocking=True, new=0):
        " shelve.open creates file if it doesn't exist "
        " if blocking set == True, if can't get lock, returns immediately with value 0 "
        #if not new:
        #   if not os.path.exists(self.filename):
        #       self.error("unable to find %s" % self.filename)
        #       self.error("current directory %s" % os.getcwd())
        #       return 0
        if new: GB.outstream("openDatabase: creating new project file")
            
        ###print(f"spiderClasses:openDatabase:260: blocking={blocking}, locked: {self.lock.locked()}")
        # If it's already locked, SPIRE will freeze
        if self.lock.locked():
            self.error("openDatabase: unable to acquire lock")
            return 0

        lock_acquired = self.lock.acquire(blocking)
        ###print(f"spiderClasses:openDatabase:262: acquire={lock_acquired}")
        if blocking == False and lock_acquired == False:
            self.error("openDatabase: unable to acquire lock")
            return 0
        ...
        #try:
          ...
          ###self.database = shelve.open(os.path.splitext(self.filename)[0])  # shelve -> dbm will add extension
          ###self.lock.release()  #### TESTING
        #except Exception as e:
          #self.error(f"openDatabase error: {e}")
          #return 0

        ###print("spiderClasses:openDatabase:278: Opening:", self.filename)
        self.database = shelve.open(os.path.splitext(self.filename)[0])  # shelve -> dbm will add extension
        #print("spiderClasses:openDatabase:280: self.database:", self.database, type(self.database) )
        ##print("spiderClasses:openDatabase:281: locked:", self.lock.locked())
        ##self.lock.release()  #### TESTING
        ##print("spiderClasses:openDatabase:282: locked:", self.lock.locked())

        #try:
          #self.database = shelve.open(os.path.splitext(self.filename)[0])  # shelve -> dbm will add extension
          #print("spiderClasses:openDatabase:280: self.database:", self.database)
          #print("spiderClasses:openDatabase:281: locked:", self.lock.locked())
        #except Exception as e:
          #self.error(f"openDatabase error: {e}")
          #print("spiderClasses:openDatabase:284: locked:", self.lock.locked())
          #self.lock.release()  #### TESTING
          #print("spiderClasses:openDatabase:286: locked:", self.lock.locked())
          #return 0

        # Only if everything works do we return 1
        return 1
    ...
    def closeDatabase(self):
        try:
            ...
            self.error(f"closeDatabase error: {e}")
            return 0
...
class scrolledForm:
    ...
    def adjustSize(self):
        ...
        _, httop = self.ftop.winfo_reqwidth(), self.ftop.winfo_reqheight()
        wdmid, htmid = self.fmain.winfo_reqwidth(), self.fmain.winfo_reqheight()
        _, htbut = self.fbut.winfo_reqwidth(), self.fbut.winfo_reqheight()
    ...
    def mktopFrame(self):
        ...
        ftop = tkinter.Frame(self.top, relief=GG.frelief, borderwidth=GG.brdr,
                     background=bgd)
        titlab = tkinter.Label(ftop, text=title, font=title_font,
                       background=bgd, foreground=textcolor)
        sublab = tkinter.Label(ftop, text=subtitle, font=subtitle_font,
                       background=bgd, foreground=textcolor) #,
    ...
    def mkbuttonFrame(self):
        ...
        fbut = tkinter.Frame(self.top, relief=GG.frelief, borderwidth=GG.brdr,
                     background=bgd)
        ##print("spiderClasses:761:mkButtomFrame: self.top:\t", self.top, type(self.top) )
        ##print("spiderClasses:762:mkButtomFrame: fbut:\t\t", fbut, type(fbut) )
        self.makeButtons(fbut)
        return fbut

    def addButton(self, frame, text, command, side='left', help=None):
        b = tkinter.Button(frame, text=text, command=command)
    ...
    def mkMainFrame(self):
        ...
        for i in range(20):
            ...
            b = tkinter.Label(fr,text=s, font=GG.spid.font['large'])
            b.pack(side='top')
