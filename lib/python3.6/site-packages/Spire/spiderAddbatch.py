...
class newBatchRun:
    def __init__(self):
        ...
        self.directory = ""
        self.outputfiles = []
        self.date, self.time, self.id = spiderUtils.nowisthetime()

class addBatchRunForm:
    def __init__(self, w, newbat):
        self.win = w
        self.newbat = newbat
        f1 = tkinter.Frame(w, relief=GG.frelief, borderwidth=GG.brdr, background=GG.bgd01)
        txt = "Add a new batch run to the project"
        tkinter.Label(f1,text=txt, background=GG.bgd01).pack(padx=4, pady=4)
        f1.pack(side='top', fill='x', expand=1)

        f2 = tkinter.Frame(w, relief=GG.frelief, borderwidth=GG.brdr)
        p = 4

        # procedure name
        self.procVar = tkinter.StringVar()
        self.procVar.set(newbat.procedure)
        lbproc = tkinter.Label(f2, text="procedure name:")
        entproc = tkinter.Entry(f2, textvariable=self.procVar)
        ...
        lbout = tkinter.Button(f2, text='output files:', command=self.getoutputs)
        entout = tkinter.Entry(f2, textvariable=self.outputs)
        ...
        lbdat = tkinter.Label(f2, text="date:")
        entdat = tkinter.Entry(f2, textvariable=self.dateVar)
        lbdat.grid(row=2, column=0, padx=p, pady=p, sticky='e')
        entdat.grid(row=2, column=1, padx=p, pady=p, sticky='ew')
        lbtim = tkinter.Label(f2, text="time:")
        entim = tkinter.Entry(f2, textvariable=self.timeVar)
        ...
        fb = tkinter.Frame(w, relief=GG.frelief, borderwidth=GG.brdr, background=GG.bgd01)
        cancel = tkinter.Button(fb, text='Cancel', command=self.cancel)
        cancel.pack(side='left', padx=p, pady=p)
        ok = tkinter.Button(fb, text='OK', command=self.quit)
        ok.pack(side='right', padx=p, pady=p) 
        fb.pack(side='bottom', fill='x', expand=1)

    def getoutputs(self):
        files = spiderGUtils.getFiles(parent=self.win)
        if len(files) == 0: return
        nf = []
        for file in files:
            f = spiderUtils.projFilename(file)
            if os.sep in f:      # find which project directory it's in
                pdir, pat = spiderUtils.pathpop(f)   # split path into (head, remainder)
...
def addNewBatchRun():
    ...
    win = spiderGUtils.newWindow("Add new data to project")
    if win == 0: return ""
    nbr = newBatchRun() # create a new batch run object

    # display the form
    addBatchRunForm(win, nbr)  #### previously set to 'abr'
