...
def create():
    ...
    sw = spiderGUtils.newWindow('Sequence editor')
    if sw != 0:
        Sequence(sw, batchfiles=batlist)  #### previously set to 'seq'
...
class Sequence:
    def __init__(self, parent, batchfiles=None):
        ...
        ftop = tkinter.Frame(parent, relief=GG.frelief, borderwidth=GG.brdr,
                     background=GG.bgd01)
        fmid = tkinter.Frame(parent)
        fbot = tkinter.Frame(parent, relief=GG.frelief, borderwidth=GG.brdr,
                     background=GG.bgd01)

        self.sequence_file = ""

        # top frame = Menubar?
        self.label = tkinter.Label(ftop, text="Batch file sequence editor")
        self.label.pack()
        ftop.pack(side='top', fill='x', expand=0)

        # bottom frame with buttons
        loadb = tkinter.Button(fbot, text='Load\nSeq.', command=self.load)
        saveb = tkinter.Button(fbot, text='Save\nSeq.', command=self.save)
        runseqb = tkinter.Button(fbot, text='Run\nSeq.', command=self.runseq)

        browseb = tkinter.Button(fbot, text='Add\nBatch', command=self.browse)
        removeb = tkinter.Button(fbot, text='Remove\nBatch', command=self.remove)
        clearb = tkinter.Button(fbot, text='Clear', command=self.clearall)
        cancelb = tkinter.Button(fbot, text='Done', command=self.cancel)
        px = py = 4
        ...
        self.batchlistbox.configure(selectmode=tkinter.EXTENDED)

        addbut = tkinter.Button(fmid, text='>>', command=self.add)
        ...
        self.seqlistbox.configure(selectmode=tkinter.EXTENDED)
        # for drag'n'drop
    ...
    def browse(self):
        ...
        filenames = spiderGUtils.getFiles()  # multiple selections allowed
        if len(filenames) == 0:
            return
        newfilenames = []
        for file in filenames:
            newfilenames.append(spiderUtils.projFilename(file))
    ...
    def save(self):
        filename = spiderGUtils.askSaveAsFilename()
        if filename == "":
            return
        ...
        spiderUtils.fileWriteLines(filename, itemlist)
        seqfile = os.path.basename(filename)
        self.label.configure(text=seqfile)
        self.sequence_file = filename
