...
import copy,os,re
##import stat
##from string import split, strip, upper, lower, count, find
import sys
...
from . import spiderClasses  #### from .spiderClasses import scrolledForm

re_regs = re.compile(r'^[xX][0-9][0-9]\s*=')  # looks for "x44 =" patterns
# named regs may have underscores, hyphens, dots. No spaces.
re_namedregs = re.compile(r'\[[\._\w-]+\] *=')  # "[namedreg] = " patterns
re_anyreg = re.compile(r'[xX][0-9][0-9]')  # looks for any register patterns
re_anynamedreg = re.compile(r'\[[\._\w-]+\]')  # "[any]" patterns

re_namedregpattern = re.compile(r'{\*+\[[\._\w-]+\]}')  # "{**[any]}" patterns
re_regpattern = re.compile(r"{\*+[xX]\d\d}")  # looks for {****x22} patterns
re_noreg = re.compile(r"[.\w_-]+[*]+")  # mic*** no registers: underscores, dots, minus allowed in fnames
re_stars = re.compile(r"[*]+")   # one or more *'s
re_group = re.compile(r"-- *[\w ?!#$%^&*()+=':;]+ *--")     #  looks for "-- text --"
#re_group = re.compile(r'-- *[\w ]+ *--')     #  looks for "-- text --"
#re_group = re.compile(r'--[^\-]+--')     #  looks for "-- text --"
re_FRG  = re.compile(r'[Ff][Rr] [GgLl]')
#re_FRL  = re.compile(r'[Ff][Rr] [Ll]')
re_Nend = re.compile(r"\d{1,}$") # gets numbers at the end of a string
...
def isSpiderFloat(x):
    try:
        float(x)  #### previously assigned to 'z'
        return 1
    except:
...
def replacePatterns(oldpattern, newpath, deleteExt=0):
    ...
    P = oldpattern.split(os.sep)
    P.reverse()
    N = newpath.split(os.sep)
    N.reverse()
    lenP = len(P)
    lenN = len(N)
    nelements = lenP
...
def fname2pattern(oldpattern, filename):
    ...
    new = ""
    for pattern in all:
        nstars = pattern.count("*")  ### count(pattern,"*")
...
class BatchFile:
    ...
    def __init__(self, filename, B):
        ...
        self.fullname = spiderUtils.truepath(filename)
        ...
        self.lastmodified = spiderUtils.lastModified(filename)
        #self.printGroups()
    ...
    def getBatchHeader(self):
        ...
        for line in self.B:
            if len(line) > 0:
                ...
                if line.upper().find(GB.end_of_header) > -1:
                    found = 1
                    break
            H.append(line)
        if not found:
    ...
    def findRegister(self, line):
        ...
        if re_regs.match(line) or re_namedregs.match(line):
            ...
            try:
                float(eval(value))  #### previously assigned to 'f'
                return line
            except:
                return None
    ...
    def getRegisters(self):
        ...
        for item in S:
            ...
            if start_comment < 0:
                ...
                value = s.strip()  # strip(s)
                comment = "(NEEDS A COMMENT) %s =" % (register)
            else:
                value = s[:start_comment].strip()
                comment = s[start_comment+1:].strip()
        ...
        for i in range(n):
            ...
            if line[0] == ';':
                ...
                if b:
                    ...
                    p = b.start() + 2
                    q = b.end() - 2
                    thisgroup = line[p:q].strip()
                    files = []

            # if it's an FR command, add next line to 'files' list
            elif re_FRG.match(line):
                nline = self.header[i+1].strip()
                symbol,value,comment = spiderUtils.symbolValueComment(nline)
                if symbol == "":
                    continue
                if symbol.find("=") > -1 or value.find("=") > -1:
                    ...
                    continue
                if comment == "":
                    comment = spiderUtils.getComment(line)
                    if comment == "":
            ...
            # if it's a VAR assignment, add line to 'files' list
                ...
                """
                try:
                    symbol,value,comment = spiderUtils.symbolValueComment(line)
                    if i > 0:
...
class batchForm(spiderClasses.scrolledForm):
    """ present registers and filename strings to user for editing """
    
    def processargs(self):
        if "geometry" in self.kwargs:
            self.top.geometry(self.kwargs["geometry"])
        if "batchobj" in self.kwargs:
            self.batobj = self.kwargs["batchobj"]
        if "displayFRhelp" in self.kwargs and self.kwargs["displayFRhelp"]:
            self.helpmsg = """
        ...
        if self.batobj.message != "":
            spiderGUtils.displayError(self.batobj.filename+'\n'+ self.batobj.message)
    ...
    def makeButtons(self, bfr):
        ...
        execute = tkinter.Button(bfr, text='Execute!', activebackground=execGreen,
    ...
    def mkMainFrame(self):
        ...
        if len(bat.registers) > 0:
            ...
            for reg in bat.registers:
                ...
                x = spiderGUtils.fontMeasure(name)
        ...
        for g in bat.groups:
            ...
            for symobj in filelist:
                ...
                if isSpiderFloat(value):
                    ...
                    b = tkinter.Button(ffr, text=label,
                               command=spiderUtils.Command(self.askfile,f,label))
    ...
    def askfile(self,entry,label=None):
        ...
        if hasattr(self.batobj, 'dir'):
            spiderUtils.chdir(self.batobj.dir)

        if label.lower().find('directory') > -1:
            spiderGUtils.getDir2(entry)
        else:
            if not spiderGUtils.getFile(entry):
                return
        ...
        if p:
            ...
            spiderGUtils.displayError('SPIDER may expect a template\n' + \
    ...
    def save(self, filename=None, saveas=0, quit=1):
        ...
        #print saveas
        #kastmod = spiderUtils.lastModified(filename)
        #print kastmod
        #print self.batobj.lastmodified

        if not saveas:
            lastmod = spiderUtils.lastModified(filename) # returns zero if file does not exist
            if lastmod != 0 and lastmod > self.batobj.lastmodified:
                ...
                updateres = spiderGUtils.askYesorNo(msg, "File modified")
                #print updateres
                if updateres:
        ...
        spiderUtils.fileWriteLines(filename, B)
    ...
    def saveas(self):
        filename = spiderGUtils.askSaveAsFilename()  #askfilename()
        if filename == "":
            return
        self.save(filename=filename, saveas=1)  #### previously assigned to 'res'
...
def getBatch(filename=None):
    ...
    if B == None: return
    if not spiderUtils.isPython(filename):
    ...
    if batdir == '.':
        batdir = GB.P.projdir
    batchForm(batchobj=bat, title=bat.filename,
                   subtitle="directory: %s" % (batdir),displayFRhelp=1)
    # (previously assigned to 'bf')
...
def updateBatch(batobj, batform):
    ...
    batchForm(batchobj=bat, geometry=geo,
                   title=bat.filename,
