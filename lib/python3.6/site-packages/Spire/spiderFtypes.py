...
def isSpiderFile(filename):
    ...
    if hdrlen == 0:
        ...
        t = struct.unpack('<23f',f)  # little-endian
        hdrlen = isSpiderHeader(t)
    ###import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:112:{inspect.stack()[0][3]}\tbigendian= '{bigendian}'")
    return hdrlen
...
_null_trans = bytes.maketrans(bytes(range(256)), bytes(range(256)))
    ...
    try:
        with open(filename, 'rb') as f:
            block = f.read(blocksize)
                ...
                #return True
            ## Count non-text bytes (not printable ASCII or whitespace)
            #non_text = sum(1 for b in block if b < 9 or (b > 13 and b < 32) or b > 126)
            #return (non_text / len(block)) <= 0.30
            return istext(block)
...
def istext(block):
    ...
    #except Exception as e:
        #import inspect
        #print("ERROR:", e)
        #print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:260:{inspect.stack()[0][3]}\t_null_trans= '{_null_trans}'")
        #print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:261:{inspect.stack()[0][3]}\ttext_characters= '{text_characters}'")
        #print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:262:{inspect.stack()[0][3]}\ts= '{s}' {type(s)}")
        #exit()
    ...
    # Adapted from https://code.activestate.com/recipes/173220-test-if-a-file-or-string-is-text-or-binary/
    if b'\x00' in block:
        # Files with null bytes are binary
        return 0
    if not block:
...
def nkeysSpiderDoc(file):
    try:
        ...
        spiderGUtils.displayError( 'unable to open %s' % (file))
        return -1
    cmd = 'tail -5 %s' % (file)
    try:
        T,err = spiderUtils.getCommandOutput(cmd)
        #T = getoutput(cmd)  
    except:
        return -1
    t = T.split('\n')
    ...
    for i in range(n,-1,-1): # start from bottom
        ...
        try:
            nkeys = int(s[:6])
            int(s[6])  #### previously assigned to 'nfs'
            return nkeys
        except:
            return -1
...
def isSpiderDocfile(file):
    ...
    while 1:
        ...
        if len(s) < 13:
            ...
            break
        try:
            int(s[0:6])   # 1st 6 chars are key  ### previously assigned to 'key'
            n = int(s[6])       # 7th char is N
            float(s[7:13])   # see if there's 1 good data value  ### previously assigned to 'f'
            isDoc = 1
            break
        except:
            isDoc = 0
...
re_proc_hdr  = re.compile(r'\[([xX][0-9][0-9],?)+\]')  # "[x12,x13,..]" patterns
re_proc_hdr2 = re.compile(r'\(([xX][0-9][0-9],?)+\)')  # "(x12,x13,..)" patterns
re_proc_hdr3 = re.compile(r'\( *(( *, *)?\[[\._\w-]+\])+ *\)')  # ([a],[b]) patterns
re_sym = re.compile(r'\?[\w ]+\?')   # "?symbol?" patterns
def isSpiderProcedurefile(file):
    ...
    """
    B = spiderUtils.fileReadLines(file)
    if B == None:
        return 0
    ...
    for i in range(maxlin):
        line = B[i]
        line = spiderUtils.stripComment(line)
        if len(line) == 0:
            continue
        ...
        # FR must be followed by "?symbol?"
            ...
            # If it's a Python file not ending in ".py", import lines lines might also begin with "fr"
            if line[0:4].lower() == "from" : return 0
            ...
            try:
                ...
                import inspect
                print(f"WARNING! {os.path.splitext( os.path.basename(__file__) )[0]}:{inspect.stack()[0][3]}: Unrecognized case")
                print(f"  file= '{file}'")
                print(f"  line= '{line}'")
                print(f"  Exception: '{e}'")
                print()
    ...
    B.reverse()
    for line in B:
        s = spiderUtils.stripComment(line).upper()
        if s == 'RE':
            has_RE = 1
            break
...
def spiderDate(t):
    ###import types
    if isinstance(t, int):  # output of time()
        t = time.asctime(time.localtime(t))
    elif isinstance(t, int):  # output of time()
        t = time.asctime(time.localtime(t))
    elif isinstance(t, float):  # output of time()
        t = time.asctime(time.localtime(t))
    elif isinstance(t, tuple): # output from localtime()
        t = time.asctime(t)
    elif isinstance(t, str):  # output from asctime()
        pass
    else:
        return ('XXX', 'XXX')
    t = t.split()
...
def getFileType(file):
    ...
    name = spiderUtils.projFilename(file)
    tup = os.stat(file)
    ...
    if istextfile(file):
        if isSpiderDocfile(file):
            ...
            if nkeys == -1:
                spiderGUtils.displayError('Unable to get last key from %s' %(file))
                return ['text']
            tup = os.stat(file)
            nbytes = tup[6]
