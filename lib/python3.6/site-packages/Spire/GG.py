...
from .spiderUtils import hasProject
from . import GB
...
#__version__ = '1.6'                      #al nov18
__version__ = '2.0'                      #trs jul25
...
#print("defaultconfigdir %s" % defaultconfigdir)
mainImage = os.path.join(defaultconfigdir, "pics/ribo1.gif") #"pics/goat.gif"   #"atom.gif")
...
__path__  = [os.path.join(os.path.dirname(defaultconfigdir), 'config')] #al
...
if webBasedir == "":
    webBasedir = 'https://spider-em.github.io/'
    #webBasedir = 'file:///usr/local/spider/docs/'
defaultEditor  = "xedit"  # ..for KDE. otherwise use "emacs"?
    

HelpURL      = os.path.join(webBasedir, "SPIDER/docs/index.html")
SpireHelpURL = os.path.join(webBasedir, "spire/spire-docs/spire.html")

# Dictionary that links extensions to external applications
if hasattr(LocalVars, 'AppDict'):
    AppDict  = LocalVars.AppDict
else:
    AppDict  = {".gnu" : 'gnuplot',".gp"  : 'gnuplot',".pdf" : 'evince',
                ".ps"  : 'gs',".tif" : 'qview', ".gif" : 'qview',".jpg" : 'qview' }

# Careful that this is accessed only by the main thread
disp_fnums = tkinter.StringVar()
...
class SysFonts:
    ...
    def __init__(self):
        self.font = {'small' : ('Helvetica',12,'normal'),
...
SysInterface and saved in $HOME/.spire
"""
class UserPrefs:
    ...
    def __init__(self):
        ...
        self.startupJweb       = 0
        self.displayProgram    = 'qview' # Or qview or xv
...
##print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:292:\thasProject={hasProject()}")
##print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:293:\tinterface.file={interface.file}")
...
###print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:298:\tinterface.file={interface.file}")
