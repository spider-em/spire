...
from .spiderUtils import hasProject, isMainThread, addExtension, docWriteLines, readdoc, fileReadLines, stripComment, threadedGUIcall
from .spiderGUtils import getFiles, askSaveAsFilename, writeUserPrefs, noProjectError
import os
...
def readFileNumbers(event=None, filenumFile=None):
    ...
    #import inspect
    #print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:64:\t{inspect.stack()[0][3]}() called by {inspect.stack()[1][3]}()")
    #print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:65:{inspect.stack()[0][3]}\tfilenumFile= '{filenumFile}'")
    if filenumFile == None:
    ...
    if a == 1:
        GB.outstream('File numbers written to ' + os.path.abspath(fname) + '\n')
...
def setFileNumbers(fnlist):
    ###import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:132:\t{inspect.stack()[0][3]}() called by {inspect.stack()[1][3]}()")
...
def useFileNumbers(filename):
    ...
    for line in lines:
        ...
        if line.find(filenumSymbol) > -1:
            ...
            if stripComment(line).find(filenumSymbol) > -1:
                found = 1
                break
    return found
