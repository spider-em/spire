...
def execBatch(batdir, batfile): 
    ...
    execdir = spiderUtils.truepath(execdir)
    ...
    useFileNums = spiderFilenums.useFileNumbers(os.path.join(execdir,batfile))
    newfilenums = None
    ...
    if useFileNums:
        ...
        if GG.prefs.useFilenumsEntry:
            ...
            if fns == None or fns == "":
                ...
                spiderFilenums.readFileNumbers(filenumFile=newfilenums)  # writes entrybox to file
                br.filenumfile = newfilenums
        else:
            spiderFilenums.readFileNumbers()  # writes to default filenums file
    ...
    ret = spiderReplace.batReplace(batfile, filenumfile=newfilenums,
    ...
    spdout = "spire.out" 
    spderr = "spire.err"  
    spiderUtils.delete(spdout)
    spiderUtils.delete(spderr)
    br.spdout = spdout
    br.spderr = spderr
    ...
    if spiderUtils.fileWriteLines(br.script, B) != 1:
        spiderExecwin.execBatchCleanup(ew=ew, success=0)
        return -1
    os.chmod(br.script, 0o777)
    ...
    # !!!!!!!!!!!!!! run SPIDER !!!!!!!!!!!!!!!!!!!!!!!!
    # --------------------------------------------------
    shellreturn = spiderUtils.getCommandOutput2(scmd)

    # now get pid from shellreturn
    a = shellreturn.find(']') # should have form: "[1] 58988"
    if a > -1:
        pid = shellreturn[a+1:]
        try:
            int(pid)  ### string.atoi(pid)
            br.pid = pid.strip()
            ew.insertpid(pid)
            #GB.outstream( "pid: " + pid + "\n")
        except:
            spiderGUtils.displayError('Problem with Process ID : ' + pid)
            spiderExecwin.execBatchCleanup(ew,0)
            return -1
    else:
        spiderGUtils.displayError('Unable to get Process ID from\n' + shellreturn)
    ...
    while 1:
        if os.path.exists(spdout):
            s = spiderUtils.fileReadLines(spdout)
            okdone = 0
            for line in s:
    ...
    for line in spdoutmsg:
        v = line.find("VERSION:")
        if v > 0:
            versionline = line[v:].strip()
            a = line.find('UNIX')
            b = line.find('ISSUED')
            if a > -1 and b > -1 and b > a+4:
                spiderversion = line[a+4:b].strip()
        elif line.find("Running:") > -1:
            runningline = line.replace("Running:", "").strip()
    ...
    if stopline != "" and stopline.find(" NORMAL ") > -1: # success
        ...
        time_end = ()
        for t in tailres:
            if t.find("COMPLETED") > -1:
                tr = t.split()
                time_end = (tr[-3],tr[-1])
                break
        if len(time_end) == 0:
            t = spiderUtils.nowisthetime()
            time_end = (t[0],t[1])
            
        # Successful run - create a batchRun object (sets b.id)
        #                  ========================
        b = spiderClasses.SpiderBatchRun() # spiderResult.readResults(br.resultfile)
        # set default values 
        ...
        if not writeResult:
            spiderUtils.delete(br.resultfile)
...
def getSpiderPID(host):   # =None, user=None):
    ...
    cmd = spiderUtils.makeCommand("ps -u %s | grep spider" % user)
    #print "getSpiderPID: " + cmd
    out,err = spiderUtils.getCommandOutput(cmd)
    if len(out) == 0:
        return []
    ...
    for item in B:
        ...
        d = s[0]
        try:
            int(d)  ### string.atoi(d)   # see if it's a digit  ### previously assigned to 'pid'
            S.append(d)
        except ValueError:
            pass      
    return S
