...
class db_upload:
    ...
    def __init__(self, database):
        ...
        f0 = tkinter.Frame(master, background=GG.bgd01, relief=GG.frelief, borderwidth=GG.brdr)
        title = "The following files and values will be written to %s" % self.db.databaseName
        tkinter.Label(f0, text=title, background=GG.bgd01).pack(side='top', padx=4, pady=4)
        f0.pack(side='top', fill='x', expand=1)
            
        f1 = tkinter.Frame(master, relief=GG.frelief, borderwidth=GG.brdr)
        row = 0
        for item in uploadfiles:
            prompt = item[2]
            _type = item[3]
            ...
            if _type == "string":
                dblabel = tkinter.Label(f1, text=prompt+":")
                dbentry = tkinter.Entry(f1, textvariable=var)
                ...
                var.set(default)
                dblabel = tkinter.Button(f1, text=prompt+":", anchor='e',
                                 command=spiderUtils.Command(self.getfile, item))
                dbentry = tkinter.Entry(f1, textvariable=var)
        ...
        fb = tkinter.Frame(master, background=GG.bgd01, relief=GG.frelief, borderwidth=GG.brdr)
        b = tkinter.Button(fb, text="Upload now",
                   command=spiderUtils.Command(self.upload, uploadfiles))
        b.pack(side='left', padx=p, pady=p)
        c = tkinter.Button(fb, text="Cancel", command=master.destroy)
    ...
    def upload(self, obj):
        ...
        if notfound:           
            ...
            nf = tkinter.Frame(self.errw, relief=GG.frelief, borderwidth=GG.brdr)
            tkinter.Label(nf, text="The following files could not be located").pack(side='top')
            for item in notfound:
                tkinter.Label(nf, text=item).pack(side='top', padx=4, pady=2)
            nf.pack(side='top', fill='x', expand=1)
                
            fb = tkinter.Frame(self.errw, background=GG.bgd01, relief=GG.frelief, borderwidth=GG.brdr)
            b = tkinter.Button(fb, text="Upload the rest",
                       command=self.go_upload)
            b.pack(side='left', padx=4, pady=4)
            c = tkinter.Button(fb, text="Cancel", command=self.errw.destroy)
