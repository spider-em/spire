...
def template2filename(template, n=0):  #numfile=None, n=None):
    ...
    if type(n) == type(1):
        ...
        n = getfilenumber(n)  # I'm not sure if this is correct?
    else:
...
def newfilename(prefix, directory=None):
    ...
    newname = prefix + str(max+1).zfill(nplaces) + ext
    return newname
...
def fixDirname(directory):
    ...
    directory = directory.replace("/tmp_mnt", "")
    return directory
...
def checkFileAccess(f):
    ...
    if os.access(f,os.R_OK) and os.access(f,os.W_OK):
        ...
        if os.path.isfile(f): mode = 0o664
        elif os.path.isdir(f): mode = 0o775
        else: return 0
        try:
            os.chmod(f,mode)
            return 1
...
def fileWriteLines(filename, B):
    ###import inspect ; print(f"{os.path.splitext( os.path.basename(__file__) )[0]}:584:\t{inspect.stack()[0][3]}() called by {inspect.stack()[1][3]}()")
...
def readdoc(filename, col):
    ...
    for item in B:
        ...
        if ';' in a:
            ...
            a = item.split()
            d = float(a[col])  ### string.atof(a[col])
            D.append(d)
    return D
...
def hasProject(attr=None):
    if not hasattr(GB,'P'):
      ###print("spiderUtils:709:hasProject:\tGB does not have P")
      return 0
    if attr == None and hasattr(GB.P,'ID'):
      ###print("spiderUtils:712:hasProject:\tGB has ID")
      return 1
    if attr != None and hasattr(GB.P,attr):
      ###print(f"spiderUtils:715:hasProject:\tGB has {attr}")
      return 1
    ###print(f"spiderUtils:717:hasProject:\tatr='{attr}'")
    return 0
...
def getBatchRun(id, outputs=None):
    ...
    if not outputs:
        ...
        if hasattr(GB.P,'T') and id in GB.P.T:
            br = GB.P.T[id]
        else:
...
def saveBatchRun(batchrun):
    ...
    ###print("spiderUtils:842:saveBatchRun: GB.P.RunList:", GB.P.RunList)
    ...
    ###print("spiderUtils:855:saveBatchRun: GB.P.RunList:", GB.P.RunList)
    ...
    ##print("spiderUtils:864:saveBatchRun: hasattr(GB,'projview'):", hasattr(GB,'projview') )
    ##if hasattr(GB,'projview') : print("spiderUtils:865:saveBatchRun: hasattr(GB.projview, 'addBatchRun':", hasattr(GB.projview, 'addBatchRun' ) )
    if hasattr(GB,'projview') and hasattr(GB.projview, 'addBatchRun'):
        GB.projview.addBatchRun(batchrun)
    ##print("spiderUtils:868:saveBatchRun: list(GB.P.T.keys()):", list(GB.P.T.keys()) )
    ##print("spiderUtils:869:saveBatchRun: GB.P.RunList:", GB.P.RunList)
...
def getCallerInfo(function=None):
    ...
    av = inspect.getargvalues(frame)   # av = (args,varargs,varkw,locals) where
...
def getCommandOutput(command):
    ...
    child.wait()  # previously assigned to 'err'
    #if err != 0:
...
def uniqueFilename(prefix=None, extension=None, length=0):
    ...
    if prefix == None: # then generate a 2-letter prefix
        uniq += string.ascii_lowercase[int( 26*random() )]
        uniq += string.ascii_lowercase[int( 26*random() )]
    else:
        uniq = prefix
