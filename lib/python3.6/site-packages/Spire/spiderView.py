...
class projectViewer:
    def __init__(self):
        w = spiderGUtils.newWindow("Project Viewer")
        if w == 0: return
        self.top = w
        ...
        self.askdelete = tkinter.IntVar() ; self.askdelete.set(1)
        self.showballoon = tkinter.IntVar() ; self.showballoon.set(1)

        # get the Project data
        P = {}   # a dictionary[id] = [batdir, batfile, date, time, fnums]
        self.batchkeys = list(GB.P.T.keys())
        self.batchkeys.sort()
        ...
        self.obsframe = tkinter.Frame(self.batchPane)
        self.headerButton = tkinter.Button(self.obsframe, text=self.hdrtxt, font=self.bfont)
        ...
        # Fill batch pane with prior jobs
        ...
        btmp = tkinter.Button(self.pframe, text = "select a batch run to view its output files",
    ...
    def addBatchRun(self, br):
        ...
        #else:
            #print("spiderView:145:addBatchRun: hasattr(self, 'top'):", hasattr(self, 'top') )
            #print("spiderView:146:addBatchRun: self.top.winfo_exists():", self.top.winfo_exists() )
    ...
    def addBatchButton(self, br):
        ...
        b = tkinter.Button(self.bframe, text=s, #relief='flat',
    ...
    def fillBatchPane(self, data):
        ...
        #print(f"spiderView:261:fillBatchPane:\tself.batchkeys={self.batchkeys}")
        #print(f"spiderView:262:fillBatchPane:\tlist(GB.P.T.keys()):", list(GB.P.T.keys()) )
        #print(f"spiderView:263:fillBatchPane:\tGB.P.RunList:", GB.P.RunList)
    ...
    def fillOutputPane(self, id):
        ...
        # get output files
        batrun = spiderUtils.getBatchRun(id, outputs=1)
        ...
        keys = list(self.outputButtons.keys())
        for k in keys:
        ...
        if noutfiles == 0:
            ...
            b = tkinter.Button(self.pframe, text=s, relief='flat', font=self.font)
        ...
        for line in B:
            ...
            b = tkinter.Button(self.pframe, text=s, relief='flat',
                       font=self.font,
    ...
    def tooManyFiles(self, nfiles):
        var = tkinter.IntVar()
        var.set(0)
        w = spiderGUtils.newWindow("That's a lot of files!")
        ...
        tkinter.Radiobutton(w, text='Show a few', value=1,
                    variable=var).pack(anchor='w')
        tkinter.Radiobutton(w, text='Show them all',
                    value=2, variable=var).pack(anchor='w')
        tkinter.Radiobutton(w, text='Cancel', value=3,
                    variable=var).pack(anchor='w')
    ...
    def viewOutput(self, filename=None):
        ...
        spiderGUtils.viewFile(filename, verbose=0)

    def plotOutput(self, filename=None):
        if not filename:
            filename = self.currentOutputFile
        print("calling callPlotData(%s)" % filename)
        spiderGUtils.callPlotData(filename)
    ...
    def resetBalloon(self):
        if self.balloon and self.showballoon.get():
            keys = list(self.batchButtons.keys())
            for id in keys:
                b = self.batchButtons[id]
                self.balloon.bind(b,self.batBalloontxt)
            keys = list(self.outputButtons.keys())
    ...
    def createMenus(self):
        ...
        self.mBar = tkinter.Frame(self.top, relief='raised', borderwidth=2)
        self.makeOptMenu()
        #self.makeFileMenu()
        self.mBar.pack(side='top', fill='x')

        # create a popup menu for the batch pane
        self.batMenu = tkinter.Menu(self.top, tearoff=0)
        ...
        self.outMenu = tkinter.Menu(self.top, tearoff=0)
    ...
    def makeFileMenu(self):
        File_button = tkinter.Menubutton(self.mBar, text='Files', underline=0,
                                 relief=self.brelief)
