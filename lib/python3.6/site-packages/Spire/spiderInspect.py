...
from .spiderUtils import hasDatabase

def inspectObject(obj, textwin=None, indent=0):
    if textwin == None:
        w = spiderGUtils.windowExists('Inspect')
        if w:
            w.destroy()
        tw = spiderGUtils.textWindow(title="Inspect", font='output')
        if tw == 0: return
    else:
        tw = textwin
    skip = indent * " "
...
def inspectProject():
    if not hasattr(GB,'P'):
        spiderGUtils.displayError('No project to view')
        return
    inspectObject(GB.P)
    
def inspectProjectFile():
    if not hasDatabase():
        spiderGUtils.displayError('unable to open project file')
        return
