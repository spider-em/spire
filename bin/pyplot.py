...
class MyGnuplotPlot:
    ...
    def __init__(self, master, args):
        ...
        self.plotstrVar = tkinter.StringVar() ; self.plotstrVar.set("")
        ...
        self.cols.set("1:3")
        self.linestyleVar = tkinter.StringVar()   ; self.linestyleVar.set('lines')
        self.gridVar      = tkinter.IntVar()      ; self.gridVar.set(0)
        # toggles plot command window
        self.messageVar   = tkinter.IntVar()      ; self.messageVar.set(0)
    ...
    def makemenu(self):
        # ------- create the menu bar -------
        self.mBar = tkinter.Frame(self.topwindow, relief='raised', borderwidth=1)
        ...
        Filebtn.menu.add_command(label='Add file', underline=0,
                                 command=self.add_data)
        ###Hovertip(add_menu, "Add file to be plotted")
        Filebtn.menu.add_command(label='Save', underline=0,
                                 command=self.save)
        Filebtn.menu.add_command(label='Print', underline=0,
                                 command=self.psprint)
        Filebtn.menu.entryconfig("Print", state="disabled")
        Filebtn.menu.add_command(label='Export to PNG', underline=0,
                                 command=self.export2png)
        Filebtn.menu.add_command(label='Export to PDF', underline=0,
                                 command=self.export2pdf)
        ...
        Optbtn.menu.styles = tkinter.Menu(Optbtn.menu, tearoff=0)
        ...
        # replot button
        replot = tkinter.Button(self.mBar, text='Replot', command=self.replot)
    ...
    def add_data(self, datafiles=None):
        if datafiles == None or len(datafiles) == 0:
            datafiles = filedialog.askopenfilename(multiple=1)
        if datafiles == None or len(datafiles) == 0:
            return
    ...
    def checkColRange(self,yrange):
        ...
        if yrange.find(","): # may be multiple ranges
            ...
            for s in cs:
                ...
                ss = s.split(":")
                try:
                    int(ss[0])  # previously assigned to i
                    int(ss[1])  # previously assigned to i
                except:
                    return -1
            return 0
        V = yrange.split(":")
        for item in V:
            if item != '':
                try:
                    int(item)  # previously assigned to i
                except ValueError:
                    return -1
        return 0
        
    def rangeError(self):
        messagebox.showerror('Error', 'Entries must be of the form:\n\tLo:Hi\n where Lo and Hi are numbers')
    ...
    def messageWindow(self):
        ...
        if showWindow:
            if hasattr(self, 'msgwindow') and self.msgwindow.winfo_exists():
                ...
                self.msgwindow = tkinter.Toplevel(self.topwindow)
                self.msglabel = tkinter.Label(self.msgwindow,
    ...
    def displayPlotString(self, plotstr="Show gnuplot commands here"):
        ...
        if hasattr(self, 'msgwindow') and self.msgwindow.winfo_exists():
            ...
            self.msgwindow = tkinter.Toplevel(self.topwindow)
            self.msglabel = tkinter.Label(self.msgwindow, textvariable=self.plotstrVar)
            self.msglabel.pack()
